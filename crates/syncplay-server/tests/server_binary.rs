...
fn temporary_motd_file() -> std::path::PathBuf {
    ...
    let suffix = SystemTime::now()
        ...
        .as_nanos();
    path.push(format!("syncplay-rs-server-motd-{suffix}.txt"));
    path
}
...
fn wait_for_server(port: u16, child: &mut Child) -> TcpStream {
    ...
    for _ in 0..60 {
        ...
        {
            panic!("syncplay-server exited before accepting connections: {status}");
        }
    ...
    panic!("syncplay-server did not accept a connection on 127.0.0.1:{port}");
}
...
#[test]
fn syncplay_server_binary_handles_legacy_password_motd_hello_and_list() {
    ...
    let mut child = ServerChild::new(
        Command::new(env!("CARGO_BIN_EXE_syncplay-server"))
            .args([
                "--port",
                &port.to_string(),
                "--password",
            ...
            .stderr(Stdio::piped())
            .spawn()
            .expect("syncplay-server binary should spawn"),
    );
...
#[test]
fn syncplay_server_binary_accepts_legacy_python_client_hello() {
