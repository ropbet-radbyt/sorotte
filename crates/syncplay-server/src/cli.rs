...
pub(crate) fn resolve_run_config(
    ...
    let port = overrides
        .port
        .resolve_with_env(|| env_u16("SYNCPLAY_SERVER_PORT"))
        .unwrap_or(DEFAULT_SERVER_PORT);
    let room_password_salt = overrides.salt.resolve_with_env(|| {
        env_trimmed("SYNCPLAY_SALT").or_else(|| env_trimmed("SYNCPLAY_SERVER_SALT"))
    });
    let server_password_token = overrides.password.resolve_with_env(|| {
        env_trimmed("SYNCPLAY_PASSWORD").or_else(|| env_trimmed("SYNCPLAY_SERVER_PASSWORD"))
    });
    ...
    let motd_template = match overrides.motd_file {
        ...
        OptionalCliValue::Absent => env_trimmed("SYNCPLAY_SERVER_MOTD_TEMPLATE"),
    };

    let rooms_db_file = overrides
        .rooms_db_file
        .resolve_with_env(|| env_trimmed("SYNCPLAY_SERVER_ROOMS_DB_FILE").map(PathBuf::from));
    let permanent_rooms_file = overrides.permanent_rooms_file.resolve_with_env(|| {
        env_trimmed("SYNCPLAY_SERVER_PERMANENT_ROOMS_FILE").map(PathBuf::from)
    });
    let stats_db_file = overrides
        .stats_db_file
        .resolve_with_env(|| env_trimmed("SYNCPLAY_SERVER_STATS_DB_FILE").map(PathBuf::from));
    let tls_cert_path = overrides
        .tls_cert_path
        .resolve_with_env(|| env_trimmed("SYNCPLAY_SERVER_TLS_CERT_PATH").map(PathBuf::from));
...
mod tests {
    ...
    fn legacy_env_password_and_salt_have_precedence_over_rust_aliases() {
        let _env = EnvGuard::set(&[
            ("SYNCPLAY_PASSWORD", Some("legacy-password")),
            ("SYNCPLAY_SERVER_PASSWORD", Some("rust-password")),
            ("SYNCPLAY_SALT", Some("legacy-salt")),
            ("SYNCPLAY_SERVER_SALT", Some("rust-salt")),
        ]);
    ...
    fn present_without_value_suppresses_environment_defaults() {
        let _env = EnvGuard::set(&[
            ("SYNCPLAY_SERVER_PORT", Some("9001")),
            ("SYNCPLAY_PASSWORD", Some("legacy-password")),
            ("SYNCPLAY_SALT", Some("legacy-salt")),
            ("SYNCPLAY_SERVER_MOTD_TEMPLATE", Some("motd")),
            ("SYNCPLAY_SERVER_ROOMS_DB_FILE", Some("rooms.sqlite3")),
            (
                "SYNCPLAY_SERVER_PERMANENT_ROOMS_FILE",
                Some("permanent.txt"),
            ),
            ("SYNCPLAY_SERVER_STATS_DB_FILE", Some("stats.sqlite3")),
            ("SYNCPLAY_SERVER_TLS_CERT_PATH", Some("certs")),
        ]);
        let action = parse_server_cli_args([
            "--port",
            "--password",
