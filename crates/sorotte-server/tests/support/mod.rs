...
pub fn strict_release_required() -> bool {
    env_flag_enabled("SYNCPLAY_REQUIRE_SERVER_RELEASE_VERIFY")
        || env_flag_enabled("SOROTTE_SERVER_RELEASE_VERIFY")
}
...
impl ServerProcess {
    pub fn spawn(args: &[String]) -> Self {
        let mut child = Command::new(env!("CARGO_BIN_EXE_sorotte-server"))
            .args(args)
            .stdin(Stdio::null())
            .stdout(Stdio::piped())
            .stderr(Stdio::piped())
            .spawn()
            .unwrap_or_else(|error| panic!("sorotte-server binary should spawn: {error}"));
    ...
    pub fn assert_running(&mut self) {
        ...
        {
            panic!(
                "sorotte-server exited early with status {status}; stdout='{}' stderr='{}'",
                self.stdout.text(),
                self.stderr.text()
            );
        }
...
pub fn set_playlist_message(files: &[&str]) -> ProtocolMessage {
    ProtocolMessage::set(SetPayload::new().with_playlist_change(
        sorotte_protocol::PlaylistChangePayload::new(files.iter().copied()),
    ))
}
...
pub fn set_playlist_index_message(index: usize) -> ProtocolMessage {
    ProtocolMessage::set(
        SetPayload::new()
            .with_playlist_index(sorotte_protocol::PlaylistIndexPayload::new(index as i64)),
    )
}
...
pub fn expect_list_rooms(
    ...
    String,
    std::collections::BTreeMap<String, sorotte_protocol::ListUserEntry>,
> {
