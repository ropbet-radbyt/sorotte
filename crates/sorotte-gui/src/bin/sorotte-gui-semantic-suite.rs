...
fn parse_options(args: &[String]) -> Result<SuiteOptions, String> {
    ...
    while index < args.len() {
        match args[index].as_str() {
            ...
            "--help" | "-h" => {
                return Err(
                    "usage: sorotte-gui-semantic-suite [--list] [--scenario NAME ...] [--json|--text]"
                        .to_owned(),
                );
            }
            argument => {
...
fn render_summary_text(
    reports: &[sorotte_gui::semantic_smoke::GuiSemanticScenarioReport],
    failures: &[(String, String)],
) -> String {
    let mut rendered = format!(
        "suite=sorotte-gui-semantic-suite\ntotal={}\npassed={}\nfailed={}\n",
    ...
    for report in reports {
        ...
        rendered
            .push_str(&report.render(sorotte_gui::semantic_smoke::GuiSemanticOutputFormat::Text));
    }
...
fn main() {
    ...
    let options = match parse_options(&args) {
        Ok(options) => options,
        Err(error) => {
            eprintln!("sorotte-gui-semantic-suite failed: {error}");
            std::process::exit(2);
        }
    };

    let builtins = sorotte_gui::semantic_smoke::scenario_names();
    ...
    for scenario_name in scenario_names {
        ...
        match sorotte_gui::semantic_smoke::run_sorotte_gui_semantic_report(
            sorotte_gui::semantic_smoke::GuiSemanticScenarioSource::Named(scenario_name.clone()),
        ) {
