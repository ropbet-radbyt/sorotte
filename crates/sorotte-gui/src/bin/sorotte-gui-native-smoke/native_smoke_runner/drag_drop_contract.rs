...
pub(super) fn verify_drag_and_drop_contract<D: NativeGuiDriver>(
    ...
    let (mut window_child, window_handle) =
        launch_sorotte_gui_with_retry(driver, binary_path, window_launch, timeout)?;
    ...
    let playlist_drop_entry_1 = temp_root.join("drag-episode-1.mkv").display().to_string();
    let playlist_drop_entry_2 = temp_root.join("drag-episode-2.mkv").display().to_string();
    upsert_sorotte_ini_stored_client_settings_mvp_at_path(
    ...
    let (mut playlist_child, playlist_handle) =
        launch_sorotte_gui_with_retry(driver, binary_path, playlist_launch, timeout)?;
    let playlist_outcome = (|| -> Result<(), String> {
        ...
        wait_for_accessible_name(
            driver,
            playlist_handle,
            &playlist_drop_entry_1,
            step_timeout,
        )?;
        wait_for_accessible_name(
            driver,
            playlist_handle,
            &playlist_drop_entry_2,
            step_timeout,
        )?;
