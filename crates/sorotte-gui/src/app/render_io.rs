...
impl GuiDroppedFilesTarget {
    ...
    pub(super) fn load_into_shared_playlist(self, state: &SorotteGuiShellAppState) -> bool {
...
impl GuiWidgetEguiRenderer {
    ...
    {
        let paths = lookup("SOROTTE_GUI_TEST_OPEN_MEDIA_FILE_PATHS")?
            .split('|')
            .map(str::trim)
    ...
    pub(super) fn pick_player_executable(state: &SorotteGuiShellAppState) -> Option<String> {
    ...
    where
        F: Fn(&str) -> Option<String>,
    {
        lookup("SOROTTE_GUI_TEST_PLAYER_EXECUTABLE_PATH")
    ...
    where
        F: Fn(&str) -> Option<String>,
    {
        lookup("SOROTTE_GUI_TEST_STREAM_HELPER_DOWNLOADER_PATH")
            .map(|value| value.trim().to_owned())
            .filter(|value| !value.is_empty())
    }
    ...
    pub(super) fn pick_playlist_load_file(state: &SorotteGuiShellAppState) -> Option<String> {
    ...
    where
        F: Fn(&str) -> Option<String>,
    {
        lookup("SOROTTE_GUI_TEST_LOAD_PLAYLIST_PATH")
            .map(|value| value.trim().to_owned())
            .filter(|value| !value.is_empty())
    }

    pub(super) fn pick_playlist_save_file(state: &SorotteGuiShellAppState) -> Option<String> {
    ...
    where
        F: Fn(&str) -> Option<String>,
    {
        lookup("SOROTTE_GUI_TEST_SAVE_PLAYLIST_PATH")
            .map(|value| value.trim().to_owned())
            .filter(|value| !value.is_empty())
    }
    ...
    where
        F: Fn(&str) -> Option<String>,
    {
        let paths = lookup("SOROTTE_GUI_TEST_MEDIA_SEARCH_BROWSE_PATH")?
            .split('|')
            .map(str::trim)
            .filter(|value| !value.is_empty())
            .map(str::to_owned)
