...
#[cfg_attr(
    ...
    allow(
        dead_code,
        reason = "Some actions expose reducer seams to tests; test builds check that every variant has a caller."
    )
)]
#[allow(
...
pub(in crate::app) enum GuiShellAction {
    ...
    ApplyUpdateCheckResult(remote_services::UpdateCheckResult),
    ...
    CompleteDiscardConfigurationChanges(StoredClientSettings),
    CancelDiscardConfigurationChanges,
    BeginConfigurationReload,
    CompleteConfigurationReload(StoredClientSettings),
