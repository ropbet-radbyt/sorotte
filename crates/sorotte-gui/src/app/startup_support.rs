...
impl GuiClientCoreChatLoopbackBootstrap {
    pub(super) fn startup_message(&self) -> String {
        format!(
            "Startup enabled client-core chat loopback via SOROTTE_GUI_ENABLE_CLIENT_CORE_CHAT_LOOPBACK as {} in room {}.",
...
impl GuiClientCoreChatTcpBootstrap {
        ...
        F: Fn(&str) -> Option<String>,
    {
        ...
        if lookup("SOROTTE_CLIENT_HOST").is_none() {
            defaults.push("host=127.0.0.1");
        }
        if lookup("SOROTTE_CLIENT_PORT").is_none() {
            defaults.push("port=8999");
        }
        if lookup("SOROTTE_CLIENT_USERNAME").is_none() && lookup("SOROTTE_CLIENT_NAME").is_none() {
            defaults.push("user=gui-user");
        }
        if lookup("SOROTTE_CLIENT_ROOM").is_none() {
        ...
        format!(
            "Startup enabled client-core chat TCP via SOROTTE_GUI_ENABLE_CLIENT_CORE_CHAT_TCP for {} as {} in room {}.",
            self.host_arg(),
            self.username,
            self.room,
        ) + &defaults_suffix
...
impl GuiStartupPublicServerSource {
    ...
    where
        F: Fn(&str) -> Option<String>,
    {
        if let Some(path) = lookup("SOROTTE_GUI_REFRESH_PUBLIC_SERVERS_PATH")
            .map(|value| value.trim().to_owned())
            .filter(|value| !value.is_empty())
        {
            return Some(Self::FilePath(path));
        }
        lookup("SOROTTE_GUI_REFRESH_PUBLIC_SERVERS")
            .map(|value| value.trim().to_owned())
            .filter(|value| !value.is_empty())
    ...
    pub(super) fn startup_message(&self, server_count: usize) -> String {
        ...
        match self {
            Self::FilePath(path) => format!(
                "Startup loaded {server_count} {noun} from SOROTTE_GUI_REFRESH_PUBLIC_SERVERS_PATH ({path})."
            ),
            Self::InlineEnv => format!(
                "Startup loaded {server_count} {noun} from SOROTTE_GUI_REFRESH_PUBLIC_SERVERS."
            ),
        }
    }
}
...
impl GuiStartupPlayerIpcSource {
    ...
    where
        F: Fn(&str) -> Option<String>,
    {
        if let Some(path) = lookup("SOROTTE_CLIENT_MPV_IPC_PATH")
            .map(|value| value.trim().to_owned())
            .filter(|value| !value.is_empty())
        {
    ...
    pub(super) fn missing_startup_message() -> String {
        "Startup has no explicit mpv JSON IPC path. The GUI will use the saved playerPath when it points to mpv; otherwise set SOROTTE_CLIENT_MPV_IPC_PATH or SOROTTE_MPV_IPC_PATH to attach an mpv JSON IPC endpoint.".to_owned()
    }
}
...
where
    F: Fn(&str) -> Option<String>,
{
    if !env_flag_enabled_lookup(&lookup, "SOROTTE_GUI_ENABLE_CLIENT_CORE_CHAT_TCP")? {
        return Ok(None);
    }

    Ok(Some(GuiClientCoreChatTcpBootstrap {
        host: lookup("SOROTTE_CLIENT_HOST").unwrap_or_else(|| "127.0.0.1".to_owned()),
        port: env_port_lookup(&lookup, "SOROTTE_CLIENT_PORT")?.unwrap_or(8999),
        username: lookup("SOROTTE_CLIENT_USERNAME")
            .or_else(|| lookup("SOROTTE_CLIENT_NAME"))
            .unwrap_or_else(|| "gui-user".to_owned()),
        room: lookup("SOROTTE_CLIENT_ROOM").unwrap_or_else(|| "gui-demo".to_owned()),
    }))
}
...
where
    F: Fn(&str) -> Option<String>,
{
    if !env_flag_enabled_lookup(&lookup, "SOROTTE_GUI_ENABLE_CLIENT_CORE_CHAT_LOOPBACK")? {
        return Ok(None);
    }
