...
use super::{
    ...
    run_sorotte_gui_semantic_cli_from_lookup, run_sorotte_gui_semantic_report,
    run_sorotte_gui_semantic_report_from_lookup,
};
...
fn gui_semantic_scenario_runner_reports_named_results_from_lookup() {
    assert_eq!(
        gui_semantic_scenario_name_from_lookup(|name| {
            (name == "SOROTTE_GUI_SEMANTIC_SCENARIO")
    ...
    let report = run_gui_semantic_scenario_from_lookup(|name| {
        (name == "SOROTTE_GUI_SEMANTIC_SCENARIO").then(|| "configuration-surface-flow".to_owned())
    })
    ...
    let json_report = run_sorotte_gui_semantic_cli_from_lookup(|name| match name {
        "SOROTTE_GUI_SEMANTIC_SCENARIO" => Some("configuration-surface-flow".to_owned()),
        "SOROTTE_GUI_SEMANTIC_OUTPUT" => Some("json".to_owned()),
        _ => None,
    })
    ...
    assert!(
        gui_semantic_output_format_from_lookup(|name| {
            (name == "SOROTTE_GUI_SEMANTIC_OUTPUT").then(|| "yaml".to_owned())
        })
    ...
    script_path.push(format!(
        "sorotte-gui-semantic-scenario-{}-{unique_id}.txt",
        std::process::id()
    ));
    std::fs::write(
        &script_path,
...
assert-label\tpublic-servers:row:0\tMirror\n",
    ...
    let script_report = run_gui_semantic_scenario_from_lookup(|name| match name {
        "SOROTTE_GUI_SEMANTIC_SCENARIO_PATH" => Some(script_path_string.clone()),
        "SOROTTE_GUI_SEMANTIC_SCENARIO" => Some("configuration-surface-flow".to_owned()),
        _ => None,
    })
...
#[test]
fn sorotte_gui_semantic_cli_wrapper_renders_lookup_output() {
    let output = run_sorotte_gui_semantic_cli_from_lookup(|name| match name {
        "SOROTTE_GUI_SEMANTIC_SCENARIO" => Some("configuration-surface-flow".to_owned()),
        "SOROTTE_GUI_SEMANTIC_OUTPUT" => Some("json".to_owned()),
        _ => None,
    })
...
#[test]
fn sorotte_gui_semantic_cli_wrapper_runs_explicit_args() {
    let output = run_sorotte_gui_semantic_cli_from_args([
        "--scenario",
        "runtime-chat-flow",
        "--format",
        "json",
    ...
    let listed = run_sorotte_gui_semantic_cli_from_args(["--list"])
    ...
    append_script_path.push(format!(
        "sorotte-gui-semantic-append-{}-{unique_id}.txt",
        std::process::id()
    ));
    std::fs::write(
        &append_script_path,
...
assert-value\tconfig:Connection:Host\toverride.example\n",
    ...
    let appended = run_sorotte_gui_semantic_cli_from_args([
        "--scenario",
        "configuration-surface-flow",
        "--append-script",
    ...
    let described =
        run_sorotte_gui_semantic_cli_from_args(["--describe-scenarios", "--format", "json"])
...
#[test]
fn sorotte_gui_semantic_report_wrapper_returns_structured_lookup_output() {
    let report = run_sorotte_gui_semantic_report_from_lookup(|name| match name {
        "SOROTTE_GUI_SEMANTIC_SCENARIO" => Some("configuration-surface-flow".to_owned()),
        _ => None,
    })
...
#[test]
fn sorotte_gui_semantic_report_wrapper_runs_persistence_reset_flow() {
...
#[test]
fn sorotte_gui_semantic_report_wrapper_runs_player_setup_flow() {
...
#[test]
fn sorotte_gui_semantic_report_wrapper_runs_inline_script() {
    let report = run_sorotte_gui_semantic_report(GuiSemanticScenarioSource::InlineScript(
        "\
