...
{
    lookup("SOROTTE_GUI_SEMANTIC_SCENARIO")
}
...
where
    F: Fn(&str) -> Option<String>,
{
    lookup("SOROTTE_GUI_SEMANTIC_SCENARIO_PATH")
}
...
where
    F: Fn(&str) -> Option<String>,
{
    parse_gui_semantic_output_format(lookup("SOROTTE_GUI_SEMANTIC_OUTPUT").as_deref())
}

pub(super) fn run_sorotte_gui_semantic_cli_from_args_impl<I, S>(
    args: I,
    ...
    S: AsRef<str>,
{
    ...
    if describe_scenarios {
        ...
        return Ok(Some(super::run_sorotte_gui_semantic_scenario_catalog(
            format,
        )));
    }
    ...
    if let Some(append_script_path) = append_script_path {
        return match selection {
            Some(GuiSemanticScenarioSource::Named(name)) => {
                super::run_sorotte_gui_semantic_report_from_named_with_append_script_path(
                    &name,
                    &append_script_path,
                )
    ...
    selection
        .map(|selection| super::run_sorotte_gui_semantic_output(selection, format))
        .transpose()
}
    ...
    F: Fn(&str) -> Option<String>,
{
    if let Some(path) = gui_semantic_scenario_script_path_from_lookup(&lookup) {
        return super::run_sorotte_gui_semantic_report(GuiSemanticScenarioSource::ScriptPath(path))
            .map(Some);
    }
    let Some(name) = gui_semantic_scenario_name_from_lookup(lookup) else {
        return Ok(None);
    };
    super::run_sorotte_gui_semantic_report(GuiSemanticScenarioSource::Named(name)).map(Some)
}
