...
pub(in crate::app) fn probe_stream_helper_runtime_snapshot(
        ...
        && (downloader_probe.effective_path.is_none() || js_runtime_probe.effective_path.is_none())
    {
        return status_snapshot(
            ...
            Some(
                "This URL needs yt-dlp and Deno to be visible to the already-running external mpv process. Install them globally or relaunch mpv from Sorotte after setup."
                    .to_owned(),
            ),
            Some(target),
            true,
