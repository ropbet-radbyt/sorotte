...
impl GuiEframeNativeHost {
    ...
    fn syncplay_window_icon() -> std::sync::Arc<egui::IconData> {
        std::sync::Arc::new(
            eframe::icon_data::from_png_bytes(include_bytes!(
                "../../../assets/icons/sorotte-icon.png"
            ))
            .expect("bundled Sorotte window icon should decode"),
        )
    }
        ...
        TOwner: GuiQueuedRuntimeOwner + Send + 'static,
    {
        ...
        let runtime_pump: Box<dyn GuiNativeRuntimePump> =
            match GuiThreadedRuntimeOwnerPump::new(handle.clone(), owner) {
                Ok(pump) => Box::new(pump),
                Err(error) => Box::new(GuiRuntimeThreadUnavailablePump::new(handle, error)),
            };
        let mut host = Self::with_runtime_and_pump(Box::new(runtime), runtime_pump);
