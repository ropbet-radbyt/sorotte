...
impl GuiNativeApp {
    ...
    pub(in crate::app) fn new(
        ...
        let test_drop_request = match Self::test_drop_request_from_lookup(&env_trimmed) {
            Ok(request) => request,
            Err(error) => {
                eprintln!("sorotte-gui ignored invalid drag-and-drop test injection: {error}");
                None
            }
        };
        Ok(Self {
    ...
    where
        F: Fn(&str) -> Option<String>,
    {
        let Some(raw_paths) = lookup("SOROTTE_GUI_TEST_DROP_FILE_PATHS") else {
            return Ok(None);
        };
        let paths = raw_paths
            .split('|')
        ...
        let target = lookup("SOROTTE_GUI_TEST_DROP_TARGET")
            .as_deref()
    ...
    pub(in crate::app) fn show_playback_prompt(
        ...
        let (
            ...
            GuiPlaybackPromptKind::Offset => (
                ...
                "e.g. +5, -3.5, /90, 12",
                GuiPlaybackPromptKind::Offset,
                "Offset must be a supported Sorotte offset value.",
            ),
        };
