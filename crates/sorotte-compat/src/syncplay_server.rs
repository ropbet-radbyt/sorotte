...
pub fn run_syncplay_server_fanout_roundtrip(
...
pub fn run_syncplay_server_fanout_roundtrip_with_salt(
    steps: &[ServerRuntimeScenarioStep],
...
pub fn run_syncplay_server_fanout_roundtrip_with_salt_and_motd_template(
    steps: &[ServerRuntimeScenarioStep],
    controlled_room_salt: &str,
    motd_template: Option<&str>,
) -> Result<Vec<ServerRuntimeScenarioEvent>, InteropError> {
    run_syncplay_server_fanout_roundtrip_with_overrides(
        steps,
        controlled_room_salt,
        motd_template,
        false,
...
pub fn run_syncplay_server_fanout_roundtrip_with_overrides(
    steps: &[ServerRuntimeScenarioStep],
    controlled_room_salt: &str,
    motd_template: Option<&str>,
    persistent_rooms_enabled: bool,
) -> Result<Vec<ServerRuntimeScenarioEvent>, InteropError> {
    run_syncplay_server_fanout_roundtrip_with_full_overrides(
        steps,
        controlled_room_salt,
        motd_template,
...
pub(crate) fn run_syncplay_server_fanout_roundtrip_with_full_overrides(
    steps: &[ServerRuntimeScenarioStep],
    controlled_room_salt: &str,
    motd_template: Option<&str>,
    persistent_rooms_enabled: bool,
    ...
    let motd_template_file_path = motd_template
        ...
        .map(write_syncplay_motd_template_file)
        .transpose()?;
    let persistent_rooms_db_path = if persistent_rooms_enabled {
        Some(create_temporary_syncplay_rooms_db_file_path()?)
    } else {
        None
    };
    let permanent_rooms_file_path = if permanent_rooms.is_empty() {
        ...
        Some(create_temporary_syncplay_permanent_rooms_file_path(
            permanent_rooms,
        )?)
    };
    ...
    let result = (|| {
        wait_for_syncplay_server_startup(port, &mut child)?;
        drop(port_lease);
        wait_for_syncplay_permanent_rooms_startup(port, &mut child, permanent_rooms)?;

        let mut clients: BTreeMap<String, SyncplayServerClientConnection> = BTreeMap::new();
        ...
        for step in steps {
            ...
            ensure_syncplay_server_is_running(&mut child)?;
            let is_new_client = !clients.contains_key(&step.client_id);
            if is_new_client {
                let stream = connect_syncplay_client_stream(port, &step.client_id)?;
                clients.insert(
    ...
    terminate_syncplay_server_process(&mut child);
    if let Some(motd_file_path) = motd_template_file_path {
        let _ = fs::remove_file(motd_file_path);
    }
    if let Some(rooms_db_path) = persistent_rooms_db_path {
