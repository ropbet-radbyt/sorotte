...
impl LegacyServerPythonPeerHarness {
    ...
    pub fn start_peer_connected_with_timeout(
        ...
        let status = self.wait_for_peer_connected(timeout)?;
        Self::parse_peer_snapshot(&status)
    }

    pub fn wait_for_peer_observed_user_presence(
        &mut self,
        username: &str,
        timeout: Duration,
    ) -> Result<LegacyPythonPeerSnapshot, InteropError> {
        ...
        let status = self.send_peer_command_and_wait(
            &json!({
                "command": "wait_for_user_presence",
                "username": username,
                "timeoutSeconds": timeout.as_secs_f64(),
            }),
            python_peer_observation_response_timeout(timeout),
            "user-present",
        )?;
        Self::parse_peer_snapshot(&status)
    }
    ...
    pub fn set_peer_ready(&mut self, ready: bool) -> Result<(), InteropError> {
        self.ensure_peer_connected()?;
        self.send_peer_command_and_wait(
            &json!({
                "command": "set_ready",
                "ready": ready,
            }),
