...
#[test]
fn syncplay_python_same_filename_matches_client_core_on_edge_cases() {
    let pairs = [
    ...
    for ((left, right), legacy_result) in pairs.iter().zip(legacy_results) {
        let rust_result = ClientSession::same_filename(left, right);
        assert_eq!(
...
#[test]
fn syncplay_python_same_filesize_matches_client_core_on_edge_cases() {
    let pairs = vec![
    ...
    assert_eq!(legacy_results.len(), pairs.len());
    for ((left, right), legacy_result) in pairs.iter().zip(legacy_results) {
        let rust_result = ClientSession::same_filesize(left, right);
        assert_eq!(
            rust_result, legacy_result,
...
#[test]
fn syncplay_python_same_fileduration_matches_client_core_on_edge_cases() {
    let pairs = vec![
        (10.49, 12.49),
        (10.49, 13.49),
        (1.5, 4.5),
    ...
    assert_eq!(legacy_results.len(), pairs.len());
    for ((left, right), legacy_result) in pairs.iter().zip(legacy_results) {
        let rust_result = ClientSession::same_fileduration(*left, *right);
        assert_eq!(
            rust_result, legacy_result,
...
#[test]
fn syncplay_python_same_fileduration_with_config_overrides_matches_client_core_on_edge_cases() {
    ...
    for (scenario_name, show_duration_notification, different_duration_threshold) in scenarios {
        let legacy_results = match run_python_same_fileduration_batch_with_overrides(
            ...
            Ok(results) => results,
            Err(InteropError::SyncplayCheckoutMissing(_))
        ...
        assert_eq!(legacy_results.len(), pairs.len());
        for ((left, right), legacy_result) in pairs.iter().zip(legacy_results) {
            let rust_result = ClientSession::same_fileduration_with_overrides(
                *left,
                *right,
...
#[test]
fn syncplay_python_privacy_file_payload_batch_matches_client_core_behavior() {
    let cases = vec![
        (
            json!({
        ...
        cases.iter().zip(legacy_results)
    {
        ...
        let filename_mode = PrivacyMode::from_syncplay_name(filename_privacy_mode)
            .expect("filename privacy mode should map to Rust mode");
        let filesize_mode = PrivacyMode::from_syncplay_name(filesize_privacy_mode)
...
#[test]
fn syncplay_client_chat_send_contract_matches_client_core_behavior() {
    let cases = vec![
        SyncplayClientChatSendContractCase {
    ...
    let legacy_results = match run_python_syncplay_client_chat_send_contract_batch(&cases) {
        Ok(results) => results,
        Err(err) if syncplay_client_protocol_prerequisites_missing(&err) => {
            eprintln!(
...
#[test]
fn syncplay_client_set_file_contract_matches_client_core_behavior() {
    let legacy_contract = match run_python_syncplay_client_set_file_contract_probe() {
...
#[test]
fn syncplay_client_user_file_metadata_contract_matches_client_core_behavior() {
    let legacy_probe = match run_python_syncplay_client_user_file_metadata_probe() {
        Ok(probe) => probe,
        Err(err) if syncplay_client_protocol_prerequisites_missing(&err) => {
            eprintln!(
