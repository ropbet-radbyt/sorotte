...
pub(crate) fn prepare_syncplay_server_request_line(
    request_line: &str,
        ...
        && !hello.contains_key("features")
    {
        ...
        hello.insert(
            "features".to_owned(),
            syncplay_default_features_for_version(version),
        );
    }
    Ok(serde_json::to_string(&request_value)?)
}

fn syncplay_default_features_for_version(version: &str) -> Value {
    json!({
...
pub(crate) struct SyncplayServerPortLease {
    port: u16,
...
pub(crate) fn reserve_syncplay_server_port() -> Result<SyncplayServerPortLease, InteropError> {
    let lock_path = syncplay_repo_root_dir()
        .join("target")
        .join("sorotte-legacy-server-startup.lock");
    reserve_syncplay_server_port_with_lock(&lock_path, SYNCPLAY_SERVER_STARTUP_LOCK_WAIT, || {})
}

pub(crate) fn reserve_syncplay_server_port_with_lock<F>(
    lock_path: &Path,
    lock_timeout: Duration,
    mut on_contention: F,
) -> Result<SyncplayServerPortLease, InteropError>
where
    F: FnMut(),
{
    let thread_guard = SYNCPLAY_SERVER_STARTUP_LOCK
    ...
    let lock_parent = lock_path.parent().ok_or_else(|| {
        std::io::Error::new(
            ...
            "Python server startup lock must have a parent directory",
        )
    })?;
        ...
        match process_guard.try_lock() {
            ...
            Err(fs::TryLockError::WouldBlock) => {
                return Err(InteropError::Io(std::io::Error::new(
                    ...
                    "timed out waiting for the Python server startup lock",
                )));
            }
...
pub(crate) fn write_syncplay_motd_template_file(template: &str) -> Result<PathBuf, InteropError> {
...
pub(crate) fn create_temporary_syncplay_rooms_db_file_path() -> Result<PathBuf, InteropError> {
    let unique_suffix = SystemTime::now()
        .duration_since(UNIX_EPOCH)
        .unwrap_or_default()
        .as_nanos();
...
pub(crate) fn create_temporary_syncplay_permanent_rooms_file_path(
    permanent_rooms: &[&str],
...
pub(crate) fn wait_for_syncplay_server_startup(
    port: u16,
    child: &mut Child,
) -> Result<(), InteropError> {
    let startup_deadline = Instant::now() + SYNCPLAY_SERVER_START_TIMEOUT;
    while Instant::now() <= startup_deadline {
        if let Some(status) = child.try_wait()? {
            let (stdout, stderr) = collect_child_pipes(child);
            return Err(InteropError::SyncplayServerExited {
    ...
    Err(InteropError::SyncplayServerStartTimeout {
        port,
        stdout: String::new(),
        stderr: String::new(),
    })
...
pub(crate) fn wait_for_syncplay_permanent_rooms_startup(
    port: u16,
    child: &mut Child,
    permanent_rooms: &[&str],
) -> Result<(), InteropError> {
    ...
    let mut connection = SyncplayServerClientConnection {
        stream: connect_syncplay_client_stream(port, "compat-room-startup-probe")?,
    ...
    while Instant::now() <= startup_deadline {
        ensure_syncplay_server_is_running(child)?;
        for line in drain_syncplay_client_lines(&mut connection)? {
            if syncplay_list_contains_permanent_rooms(&line, permanent_rooms) {
                return close_startup_probe(connection, port, child);
            }
        }
    ...
    Err(InteropError::SyncplayServerPersistentRoomsStartTimeout {
        port,
...
fn close_startup_probe(
    mut connection: SyncplayServerClientConnection,
    port: u16,
    child: &mut Child,
) -> Result<(), InteropError> {
    connection.stream.shutdown(Shutdown::Write)?;
    let disconnect_deadline = Instant::now() + SYNCPLAY_SERVER_START_TIMEOUT;
    ...
    Err(InteropError::SyncplayServerStartupProbeDisconnectTimeout { port })
}

fn syncplay_list_contains_permanent_rooms(line: &str, permanent_rooms: &[&str]) -> bool {
...
pub(crate) fn ensure_syncplay_server_is_running(child: &mut Child) -> Result<(), InteropError> {
    if let Some(status) = child.try_wait()? {
        let (stdout, stderr) = collect_child_pipes(child);
        return Err(InteropError::SyncplayServerExited {
            exit_code: status.code(),
...
pub(crate) fn connect_syncplay_client_stream(
    port: u16,
    client_id: &str,
        ...
        match TcpStream::connect(("127.0.0.1", port)) {
            Ok(stream) => {
                stream.set_nodelay(true).map_err(|source| {
                    InteropError::SyncplayClientConnectionInit {
            ...
            Err(source) => {
                if Instant::now() >= connect_deadline {
                    return Err(InteropError::SyncplayClientConnectionInit {
                        client_id: client_id.to_owned(),
                        source,
                    });
                }
...
pub(crate) fn collect_syncplay_server_step_outputs(
    ...
    loop {
        ...
        for (client_id, connection) in clients.iter_mut() {
            let lines = drain_syncplay_client_lines(connection)?;
            if lines.is_empty() {
                continue;
            }
            saw_new_output = true;
...
pub(crate) fn drain_syncplay_client_lines(
    connection: &mut SyncplayServerClientConnection,
...
pub(crate) fn terminate_syncplay_server_process(child: &mut Child) {
...
pub(crate) fn repo_local_syncplay_checkout_dir() -> PathBuf {
...
pub(crate) fn configured_syncplay_checkout_dir() -> Option<PathBuf> {
...
pub(crate) fn syncplay_checkout_bootstrap_lock() -> &'static Mutex<()> {
    SYNCPLAY_BOOTSTRAP_LOCK.get_or_init(|| Mutex::new(()))
}

pub(crate) fn acquire_syncplay_checkout_process_lock<F>(
    checkout_path: &Path,
    timeout: Duration,
    mut on_contention: F,
    ...
    F: FnMut(),
{
    let parent = checkout_path.parent().ok_or_else(|| {
        std::io::Error::new(
            std::io::ErrorKind::InvalidInput,
            "Syncplay checkout must have a parent directory",
        )
    })?;
    ...
    let checkout_name = checkout_path.file_name().ok_or_else(|| {
        std::io::Error::new(
            std::io::ErrorKind::InvalidInput,
            "Syncplay checkout must have a file name",
        )
    })?;
        ...
        match lock_file.try_lock() {
            ...
            Err(fs::TryLockError::WouldBlock) => {
                ...
                if Instant::now() >= deadline {
                    return Err(InteropError::Io(std::io::Error::new(
                        std::io::ErrorKind::TimedOut,
                        "timed out waiting for the Syncplay checkout bootstrap lock",
                    )));
                }
                thread::sleep(Duration::from_millis(10));
            }
...
pub(crate) fn syncplay_checkout_is_ready(path: &Path) -> bool {
    path.join("syncplayServer.py").is_file()
}

pub(crate) fn remove_syncplay_checkout_path_if_present(path: &Path) -> Result<(), InteropError> {
    if !path.exists() {
        return Ok(());
    }
    if path.is_dir() {
...
pub(crate) fn bootstrap_repo_local_syncplay_checkout(path: &Path) -> Result<(), InteropError> {
    remove_syncplay_checkout_path_if_present(path)?;
    ...
    let clone_result = Command::new("git")
        ...
        .arg("1")
        .arg("--branch")
        .arg(SYNCPLAY_UPSTREAM_REF)
        .arg("--single-branch")
        .arg(SYNCPLAY_UPSTREAM_REPO)
        .arg(path)
    ...
    match clone_result {
        Ok(status) if status.success() && syncplay_checkout_is_ready(path) => Ok(()),
        Ok(_) | Err(_) => {
            let _ = remove_syncplay_checkout_path_if_present(path);
            Err(InteropError::SyncplayCheckoutMissing(path.to_path_buf()))
        }
    }
}

pub(crate) fn ensure_repo_local_syncplay_checkout_with<F, G>(
...
{
    let _process_guard =
        acquire_syncplay_checkout_process_lock(legacy_checkout, lock_timeout, on_lock_contention)?;
    if syncplay_checkout_is_ready(legacy_checkout) {
        return Ok(legacy_checkout.to_path_buf());
    }
    bootstrap(legacy_checkout)?;
    if !syncplay_checkout_is_ready(legacy_checkout) {
        return Err(InteropError::SyncplayCheckoutMissing(
...
pub(crate) fn ensure_syncplay_checkout_available() -> Result<PathBuf, InteropError> {
    let configured_checkout = configured_syncplay_checkout_dir();
    if required_live_interop_enabled() && configured_checkout.is_none() {
        return Err(required_live_prerequisite_error(
            InteropError::SyncplayCheckoutMissing(repo_local_syncplay_checkout_dir()),
        ));
    }
    if let Some(legacy_checkout) = configured_checkout {
        if !legacy_checkout.is_dir() {
            return Err(required_live_prerequisite_error(
                InteropError::SyncplayCheckoutMissing(legacy_checkout),
            ));
        }
        return Ok(legacy_checkout);
    }

    let _guard = syncplay_checkout_bootstrap_lock()
        .lock()
        .unwrap_or_else(|poisoned| poisoned.into_inner());
    let legacy_checkout = repo_local_syncplay_checkout_dir();
    ensure_repo_local_syncplay_checkout_with(
        &legacy_checkout,
        SYNCPLAY_BOOTSTRAP_LOCK_WAIT,
        || {},
        bootstrap_repo_local_syncplay_checkout,
    )
    .map_err(required_live_prerequisite_error)
}

pub fn syncplay_checkout_dir() -> PathBuf {
    configured_syncplay_checkout_dir().unwrap_or_else(repo_local_syncplay_checkout_dir)
}

pub fn syncplay_server_entry_script_path() -> PathBuf {
    syncplay_checkout_dir().join("syncplayServer.py")
}
...
pub(crate) fn stderr_indicates_missing_syncplay_prerequisites(stderr: &str) -> bool {
...
pub fn interop_prerequisites_missing(error: &InteropError) -> bool {
    ...
    match error {
        InteropError::SyncplayCheckoutMissing(_)
        | InteropError::PythonHandshakeProbeMissing(_)
        | InteropError::PythonLivePeerProbeMissing(_)
        | InteropError::SyncplayServerEntryScriptMissing(_)
        ...
        | InteropError::PythonLivePeerStartTimeout { stderr, .. } => {
            stderr_indicates_missing_syncplay_prerequisites(stderr)
        }
        _ => false,
    }
}
...
mod required_live_tests {
    use super::{
        ...
        syncplay_list_contains_permanent_rooms,
    };
    use std::ffi::OsStr;
    ...
    fn required_live_mode_wraps_optional_prerequisite_errors() {
        let missing = || InteropError::SyncplayCheckoutMissing(PathBuf::from("missing-oracle"));
        assert!(matches!(
    ...
    fn permanent_room_startup_requires_a_gui_list_snapshot_with_every_room() {
        ...
        for line in [
            ...
            assert!(!syncplay_list_contains_permanent_rooms(
                line,
                &expected_rooms
            ));
        }
