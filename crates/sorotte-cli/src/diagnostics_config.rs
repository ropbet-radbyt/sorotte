...
-> Option<ReconnectCorrectionDiagnosticsFormat> {
    if env_flag_enabled("SOROTTE_CLIENT_LOG_RECONNECT_CORRECTION_DIAGNOSTICS_JSON") {
        return Some(ReconnectCorrectionDiagnosticsFormat::Json);
    }
    if env_flag_enabled("SOROTTE_CLIENT_LOG_RECONNECT_CORRECTION_DIAGNOSTICS") {
...
-> ReconnectCorrectionDiagnosticsAlertThresholds {
    ReconnectCorrectionDiagnosticsAlertThresholds {
        action_failures_delta: env_u32(
            "SOROTTE_CLIENT_RECONNECT_CORRECTION_ALERT_ACTION_FAILURES_DELTA_THRESHOLD",
        )
        .map(u64::from),
        retry_exhaustions_delta: env_u32(
            "SOROTTE_CLIENT_RECONNECT_CORRECTION_ALERT_RETRY_EXHAUSTIONS_DELTA_THRESHOLD",
        )
        .map(u64::from),
        disables_after_repeated_mismatches_delta: env_u32(
            "SOROTTE_CLIENT_RECONNECT_CORRECTION_ALERT_DISABLES_DELTA_THRESHOLD",
        )
        .map(u64::from),
        consecutive_mismatch_cycles: env_u32(
            "SOROTTE_CLIENT_RECONNECT_CORRECTION_ALERT_CONSECUTIVE_MISMATCH_CYCLES_THRESHOLD",
        ),
        consecutive_retry_exhaustions: env_u32(
            "SOROTTE_CLIENT_RECONNECT_CORRECTION_ALERT_CONSECUTIVE_RETRY_EXHAUSTIONS_THRESHOLD",
        ),
    }
}
...
fn client_loop_diagnostics_config_from_env() -> ClientLoopDiagnosticsConfig {
    ClientLoopDiagnosticsConfig {
        log_player_telemetry: env_flag_enabled("SOROTTE_CLIENT_LOG_PLAYER_TELEMETRY"),
        log_player_drift: env_flag_enabled("SOROTTE_CLIENT_LOG_PLAYER_DRIFT_DIAGNOSTICS"),
