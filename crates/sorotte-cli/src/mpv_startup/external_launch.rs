...
pub(crate) fn player_path_requests_managed_mpv(player_path: &str) -> bool {
...
pub(crate) fn player_path_compatibility_warning_line(
...
pub(crate) fn non_mpv_player_path_ignored_by_mpv_integration_warning_line(
    overrides: &SyncplayClientArgOverrides,
) -> Option<&'static str> {
    let player_path = overrides.player_path.as_deref()?;
    if player_path_requests_managed_mpv(player_path) {
...
fn should_use_automatic_managed_mpv_for_player_path(
    argument_overrides: Option<&SyncplayClientArgOverrides>,
) -> bool {
    argument_overrides
        .and_then(|overrides| overrides.player_path.as_deref())
        .is_some_and(player_path_requests_managed_mpv)
}

pub(crate) fn should_skip_external_player_launch_due_to_mpv_integration_env() -> bool {
    explicit_mpv_ipc_path_from_env().is_some() || managed_mpv_launch_env_config_from_env().enabled
}

pub(crate) fn external_player_launch_spec_from_overrides(
    overrides: &SyncplayClientArgOverrides,
) -> Option<ExternalPlayerLaunchSpec> {
    ...
    Some(ExternalPlayerLaunchSpec { program, args })
}

pub(crate) fn spawn_external_player_if_requested(
    overrides: &SyncplayClientArgOverrides,
) -> anyhow::Result<bool> {
    let Some(spec) = external_player_launch_spec_from_overrides(overrides) else {
        return Ok(false);
    };
    if should_skip_external_player_launch_due_to_mpv_integration_env() {
        if let Some(line) = non_mpv_player_path_ignored_by_mpv_integration_warning_line(overrides) {
            eprintln!("{line}");
        }
        return Ok(false);
    }
    if should_use_automatic_managed_mpv_for_player_path(Some(overrides)) {
        return Ok(false);
    }

    let _child = spawn_external_player_from_spec(&spec)?;
    eprintln!(
        "info: launched external player '{}' (unmanaged startup path)",
        spec.program.display()
    );
    Ok(true)
}

pub(crate) fn spawn_external_player_from_spec(
    ...
    command.spawn().map_err(|error| {
        anyhow!(
            "failed to launch external player '{}' with args {:?}: {error}",
            spec.program.display(),
