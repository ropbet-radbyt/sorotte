...
pub(crate) fn apply_stored_client_settings_mvp_if_env_absent(
    ...
    let config_plan = stored_client_settings_config_plan_legacy_compatible(
        ...
        &StoredClientSettingsEnvPresence {
            ...
            port: env_port("SOROTTE_CLIENT_PORT").is_some(),
            server_password: env_trimmed("SOROTTE_CLIENT_SERVER_PASSWORD").is_some(),
            username: env_trimmed("SOROTTE_CLIENT_USERNAME").is_some()
                || env_trimmed("SOROTTE_CLIENT_NAME").is_some(),
            room: env_trimmed("SOROTTE_CLIENT_ROOM").is_some(),
            autoplay: env_trimmed("SOROTTE_CLIENT_AUTOPLAY").is_some(),
            autoplay_require_same_filenames: env_trimmed(
                "SOROTTE_CLIENT_AUTOPLAY_REQUIRE_SAME_FILENAMES",
            )
            .is_some(),
            ready_at_start: env_trimmed("SOROTTE_CLIENT_READY_AT_START").is_some(),
            shared_playlist_enabled: env_trimmed("SOROTTE_CLIENT_SHARED_PLAYLIST_ENABLED")
                .is_some(),
            pause_on_leave: env_trimmed("SOROTTE_CLIENT_PAUSE_ON_LEAVE").is_some(),
            loop_at_end_of_playlist: env_trimmed("SOROTTE_CLIENT_LOOP_AT_END_OF_PLAYLIST")
                .is_some(),
            loop_single_files: env_trimmed("SOROTTE_CLIENT_LOOP_SINGLE_FILES").is_some(),
            only_switch_to_trusted_domains: env_trimmed(
                "SOROTTE_CLIENT_ONLY_SWITCH_TO_TRUSTED_DOMAINS",
            )
            .is_some(),
            trusted_domains: env_trimmed("SOROTTE_CLIENT_TRUSTED_DOMAINS").is_some(),
            rewind_on_desync: env_trimmed("SOROTTE_CLIENT_REWIND_ON_DESYNC").is_some(),
            fastforward_on_desync: env_trimmed("SOROTTE_CLIENT_FASTFORWARD_ON_DESYNC").is_some(),
            slow_on_desync: env_trimmed("SOROTTE_CLIENT_SLOW_ON_DESYNC").is_some(),
            dont_slow_down_with_me: env_trimmed("SOROTTE_CLIENT_DONT_SLOW_DOWN_WITH_ME").is_some(),
            rewind_threshold_seconds: env_trimmed("SOROTTE_CLIENT_REWIND_THRESHOLD_SECONDS")
                .is_some(),
            fastforward_threshold_seconds: env_trimmed(
                "SOROTTE_CLIENT_FASTFORWARD_THRESHOLD_SECONDS",
            )
            .is_some(),
            slowdown_threshold_seconds: env_trimmed("SOROTTE_CLIENT_SLOWDOWN_THRESHOLD_SECONDS")
                .is_some(),
            unpause_action: env_trimmed("SOROTTE_CLIENT_UNPAUSE_ACTION").is_some(),
            autoplay_min_users: env_trimmed("SOROTTE_CLIENT_AUTOPLAY_MIN_USERS").is_some(),
            filename_privacy_mode: env_trimmed("SOROTTE_CLIENT_FILENAME_PRIVACY_MODE").is_some(),
            filesize_privacy_mode: env_trimmed("SOROTTE_CLIENT_FILESIZE_PRIVACY_MODE").is_some(),
            show_duration_notification: env_trimmed("SOROTTE_CLIENT_SHOW_DURATION_NOTIFICATION")
                .is_some(),
            show_same_room_osd: env_trimmed("SOROTTE_CLIENT_SHOW_SAME_ROOM_OSD").is_some(),
            show_osd_warnings: env_trimmed("SOROTTE_CLIENT_SHOW_OSD_WARNINGS").is_some(),
            show_noncontroller_osd: env_trimmed("SOROTTE_CLIENT_SHOW_NONCONTROLLER_OSD").is_some(),
            show_different_room_osd: env_trimmed("SOROTTE_CLIENT_SHOW_DIFFERENT_ROOM_OSD")
                .is_some(),
        },
    );
