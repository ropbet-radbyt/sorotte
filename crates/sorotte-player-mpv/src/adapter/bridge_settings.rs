...
impl MpvAdapter {
    pub(super) fn reset_syncplayintf_attachment_for_new_ipc(&mut self) {
        self.syncplayintf_script_loaded = false;
        self.syncplayintf_options_applied = false;
        self.syncplayintf_script_name = SYNCPLAYINTF_SCRIPT_NAME.to_owned();
        self.syncplayintf_bridge_instance_id = None;
        self.syncplayintf_pending_options_generation = None;
        self.syncplayintf_acknowledged_options_generation = None;
        self.syncplayintf_options_ack_error = None;
        self.syncplayintf_pending_ping_nonce = None;
        self.syncplayintf_last_heartbeat_at = None;
        self.syncplayintf_pending_heartbeat_command_id = None;
        self.syncplayintf_last_discovery_at = None;
        self.syncplayintf_lease_reacquire_required = false;
        self.syncplayintf_runtime_rediscovery_required = false;
        self.syncplayintf_runtime_recovery_attempts = 0;
        self.syncplayintf_runtime_recovery_failure = None;
        ...
        let connection_generation = self
            ...
            .unwrap_or_else(|| NEXT_SYNCPLAYINTF_ATTACHMENT.fetch_add(1, Ordering::Relaxed));
        self.syncplayintf_attachment_id =
            format!("{}-{connection_generation}", self.syncplayintf_owner_id);
    }

    pub fn syncplay_ui_settings(&self) -> &SyncplayUiSettings {
        &self.syncplay_ui_settings
    }

    pub fn last_simulated_syncplay_osd_message(&self) -> Option<&(String, SyncplayOsdKind)> {
        self.last_simulated_syncplay_osd_message.as_ref()
    }

    pub fn syncplayintf_options_ready(&self) -> bool {
        self.syncplayintf_script_loaded
            && self.syncplayintf_bridge_instance_id.is_some()
            && self.syncplayintf_options_applied
            && self.syncplayintf_pending_options_generation.is_none()
    }

    pub fn syncplayintf_script_loaded(&self) -> bool {
        self.syncplayintf_script_loaded
    }

    pub fn apply_pending_syncplayintf_options(&mut self) -> Result<(), PlayerError> {
        if !self.syncplayintf_script_loaded {
            return Err(PlayerError::OperationFailed(
                "the syncplayintf bridge is not loaded".to_owned(),
            ));
        }
        if self.syncplayintf_options_applied {
            return Ok(());
        }
        self.send_syncplayintf_options_if_loaded()
    }

    pub fn syncplay_osd_placement_restore(&self) -> Option<(String, i64)> {
        self.syncplay_osd_placement_restore.clone()
    }

    pub fn set_syncplay_osd_placement_restore(&mut self, restore: Option<(String, i64)>) {
        self.syncplay_osd_placement_restore = restore;
    }

    pub fn load_syncplayintf_script(&mut self, path: impl AsRef<Path>) -> Result<(), PlayerError> {
        ...
        if self.discover_syncplayintf_bridge(false)? {
            self.try_send_syncplayintf_options_if_pending();
            return Ok(());
        }
        ...
        self.syncplayintf_pending_options_generation = None;
        self.syncplayintf_acknowledged_options_generation = None;
        if !self.discover_syncplayintf_bridge(true)? {
            return Err(PlayerError::OperationFailed(
    ...
    pub fn configure_syncplay_ui_settings(
        &mut self,
        ...
        if placement_available && settings.should_move_osd() {
            if self.syncplay_osd_placement_restore.is_none() {
                ...
                self.syncplay_osd_placement_restore = Some(restore);
            }
            ...
            && let Some((align, margin)) = self.syncplay_osd_placement_restore.as_ref().cloned()
        {
        ...
        self.syncplay_ui_settings = settings;
    ...
    pub fn configure_bundled_sorotte_bridge(&mut self) -> SorotteBridgeHealth {
        self.configure_bundled_sorotte_bridge_inner(SYNCPLAYINTF_CONFIGURATION_RETRY_WINDOW)
    }
    ...
    pub fn sorotte_bridge_acknowledged_generation(&self) -> Option<u64> {
        self.syncplayintf_options_applied
            .then_some(self.syncplayintf_acknowledged_options_generation)
            .flatten()
    }
    ...
    pub(super) fn configure_bundled_sorotte_bridge_inner(
        ...
        let bridge_requested = self.syncplay_ui_settings.uses_syncplayintf_bridge();
        if !bridge_requested && !self.syncplayintf_script_loaded {
            return self.set_sorotte_bridge_health(SorotteBridgeHealth::Disabled);
        }

        if !self.syncplayintf_script_loaded {
            match self.discover_loaded_syncplayintf_script() {
                Ok(true) => {}
                Ok(false) if bridge_requested => {
                    ...
                    if let Err(error) = self.load_syncplayintf_script(&script_path) {
                        return self.degrade_sorotte_bridge(
        ...
        let last_error = loop {
            let error = match self.apply_pending_syncplayintf_options() {
                Ok(()) if self.syncplayintf_options_ready() => {
            ...
            if let Some(acknowledged_error) = self.syncplayintf_options_ack_error.clone() {
                last_acknowledged_error = Some(acknowledged_error);
            }
            if Instant::now() >= deadline {
                break error;
            }
            std::thread::sleep(SYNCPLAYINTF_CONFIGURATION_RETRY_INTERVAL);
        };

        let acknowledged_error = self
            .syncplayintf_options_ack_error
            .clone()
    ...
    pub(super) fn degrade_sorotte_bridge(
        ...
        self.syncplayintf_last_heartbeat_at = None;
        self.syncplayintf_pending_heartbeat_command_id = None;
        self.syncplayintf_lease_reacquire_required = kind == SorotteBridgeFailureKind::LeaseBusy;
        self.pending_chat_requests.clear();
    ...
    pub(super) fn send_syncplayintf_script_message(
        ...
        self.send_ipc_command_if_attached(json!([
            ...
            self.syncplayintf_script_name.as_str(),
            message_name,
            payload
        ]))
    }
    ...
    pub fn discover_loaded_syncplayintf_script(&mut self) -> Result<bool, PlayerError> {
        self.discover_syncplayintf_bridge(false)
    }

    pub(super) fn discover_syncplayintf_bridge(
        &mut self,
        ...
        if self.simulation_mode {
            ...
            self.syncplayintf_bridge_instance_id =
                Some("simulated-sorotte-syncplayintf".to_owned());
            self.syncplayintf_last_discovery_at = Some(Instant::now());
            return Ok(true);
        }

        let nonce = self.syncplayintf_next_ping_nonce;
        self.syncplayintf_next_ping_nonce =
            self.syncplayintf_next_ping_nonce.wrapping_add(1).max(1);
        self.syncplayintf_pending_ping_nonce = Some(nonce);
        let payload = json!({
        ...
        let attempts = if wait_for_registration {
            SYNCPLAYINTF_REGISTRATION_ATTEMPTS
        } else {
            SYNCPLAYINTF_DISCOVERY_ATTEMPTS
        };
        for _ in 0..attempts {
            let ping_accepted =
                self.send_syncplayintf_probe_message(SYNCPLAYINTF_PING_MESSAGE, &payload)?;
            ...
            if self.syncplayintf_pending_ping_nonce.is_some()
                && let Some(client) = self.ipc_client.as_mut()
            {
                let _ = client.get_property(MPV_PROPERTY_PAUSE);
                self.drain_ipc_events_if_attached();
            }
            if self.syncplayintf_pending_ping_nonce.is_none()
                && self.syncplayintf_bridge_instance_id.is_some()
            {
                self.syncplayintf_script_loaded = true;
                return Ok(true);
    ...
    pub(super) fn send_syncplayintf_options_if_loaded(&mut self) -> Result<(), PlayerError> {
        if !self.syncplayintf_script_loaded {
            return Err(PlayerError::OperationFailed(
        ...
        if self.simulation_mode {
            let generation = self.syncplayintf_next_options_generation;
            self.syncplayintf_next_options_generation = self
                .syncplayintf_next_options_generation
                .wrapping_add(1)
                .max(1);
            self.syncplayintf_options_applied = true;
            self.syncplayintf_acknowledged_options_generation = Some(generation);
            return Ok(());
        }
        ...
        let generation = match self.syncplayintf_pending_options_generation {
            ...
            None => {
                ...
                self.syncplayintf_next_options_generation = self
                    ...
                    .wrapping_add(1)
                    .max(1);
                self.syncplayintf_pending_options_generation = Some(generation);
                generation
            }
        };
        self.syncplayintf_options_applied = false;
        self.syncplayintf_options_ack_error = None;
        let payload = self.syncplay_ui_settings.syncplayintf_options_payload(
            &bridge_instance_id,
            &self.syncplayintf_owner_id,
            &self.syncplayintf_attachment_id,
            generation,
            SYNCPLAYINTF_OWNER_LEASE_MS,
        );

        self.send_syncplayintf_script_message(SYNCPLAYINTF_SET_OPTIONS_MESSAGE, &payload)?;
        if !self.syncplayintf_options_applied
            && let Some(client) = self.ipc_client.as_mut()
        {
            let _ = client.get_property(MPV_PROPERTY_PAUSE);
        ...
        if self.syncplayintf_options_applied {
            self.syncplayintf_last_heartbeat_at = Some(Instant::now());
            return Ok(());
        }
        Err(PlayerError::OperationFailed(
            self.syncplayintf_options_ack_error
                .clone()
                .unwrap_or_else(|| {
                    format!(
    ...
    pub(super) fn try_send_syncplayintf_options_if_pending(&mut self) {
        if self.syncplayintf_options_applied
        ...
        let _ = self.send_syncplayintf_options_if_loaded();
    }

    pub(super) fn ensure_syncplayintf_ready(&mut self) -> bool {
        self.try_send_syncplayintf_options_if_pending();
        self.syncplayintf_options_ready()
    }

    pub(super) fn syncplayintf_controller_payload(&self) -> Option<String> {
        Some(
            json!({
                "protocol": SYNCPLAYINTF_PROTOCOL,
                "bridgeInstanceId": self.syncplayintf_bridge_instance_id.as_deref()?,
                "ownerId": self.syncplayintf_owner_id.as_str(),
                "attachmentId": self.syncplayintf_attachment_id.as_str(),
            })
            .to_string(),
        )
    }
    ...
    pub(super) fn begin_sorotte_bridge_runtime_recovery(
        ...
        self.syncplayintf_last_heartbeat_at = None;
        self.syncplayintf_pending_heartbeat_command_id = None;
        self.syncplayintf_lease_reacquire_required = true;
        self.syncplayintf_runtime_rediscovery_required |= rediscovery_required;
        self.syncplayintf_runtime_recovery_failure = Some(SorotteBridgeFailure::new(kind, reason));
        self.pending_chat_requests.clear();
    ...
    pub(super) fn attempt_sorotte_bridge_runtime_recovery(&mut self) {
        if !matches!(self.sorotte_bridge_health, SorotteBridgeHealth::Recovering)
            || (self.syncplayintf_runtime_recovery_attempts > 0
                && self
                    .syncplayintf_last_heartbeat_at
                    .is_some_and(|last| last.elapsed() < SYNCPLAYINTF_HEARTBEAT_INTERVAL))
        {
            return;
        }
        self.syncplayintf_last_heartbeat_at = Some(Instant::now());
        ...
        let result = if self.syncplayintf_runtime_rediscovery_required {
            match self.discover_syncplayintf_bridge(false) {
                Ok(true) => {
                    self.syncplayintf_runtime_rediscovery_required = false;
                    self.send_syncplayintf_options_if_loaded()
                }
        ...
        if result.is_ok() && self.syncplayintf_options_ready() {
            let health = if self.syncplay_ui_settings.uses_syncplayintf_bridge() {
                SorotteBridgeHealth::Ready
            } else {
                SorotteBridgeHealth::Disabled
            };
        ...
        self.syncplayintf_runtime_recovery_attempts += 1;
        if let Err(error) = result {
            let acknowledged_error = self.syncplayintf_options_ack_error.clone();
            ...
            self.syncplayintf_runtime_recovery_failure =
                Some(SorotteBridgeFailure::new(kind, reason));
        }

        if self.syncplayintf_runtime_recovery_attempts >= SYNCPLAYINTF_RUNTIME_RECOVERY_ATTEMPTS {
            let failure = self
                .syncplayintf_runtime_recovery_failure
                .clone()
                .unwrap_or_else(|| {
    ...
    pub(super) fn maintain_syncplayintf_lease(&mut self) {
        self.drain_ipc_events_if_attached();
        if matches!(
            self.sorotte_bridge_health,
        ...
        if self.syncplay_ui_settings.uses_syncplayintf_bridge()
            && self
                .syncplayintf_last_discovery_at
                .is_none_or(|last| last.elapsed() >= SYNCPLAYINTF_RUNTIME_DISCOVERY_INTERVAL)
        {
            match self.discover_syncplayintf_bridge(false) {
                Ok(true) => {}
        ...
        if !self.syncplay_ui_settings.chat_input_enabled {
            self.syncplayintf_last_heartbeat_at = None;
            self.syncplayintf_pending_heartbeat_command_id = None;
            return;
        }
        if !self.syncplayintf_options_ready() {
            self.begin_sorotte_bridge_runtime_recovery(
                SorotteBridgeFailureKind::AcknowledgementTimeout,
        ...
        let Some(payload) = self.syncplayintf_controller_payload() else {
            return;
        };
        match self.send_syncplayintf_script_message(SYNCPLAYINTF_HEARTBEAT_MESSAGE, &payload) {
    ...
    pub fn release_sorotte_bridge_best_effort(&mut self) {
        ...
        if let Some((align_y, margin_y)) = self.syncplay_osd_placement_restore.take() {
        ...
        if self.syncplayintf_script_loaded
            && let Some(payload) = self.syncplayintf_controller_payload()
        {
            final_commands.push(json!([
                MPV_COMMAND_SCRIPT_MESSAGE_TO,
                self.syncplayintf_script_name.as_str(),
                SYNCPLAYINTF_RELEASE_MESSAGE,
                payload
            ]));
        }
    ...
    pub(super) fn handle_syncplayintf_options_ack(&mut self, payload: Option<&str>) {
        let parsed = payload.and_then(|payload| serde_json::from_str::<Value>(payload).ok());
        let Some(parsed) = parsed else {
            if self.syncplayintf_pending_options_generation.is_some() {
                self.syncplayintf_options_ack_error = Some(
                    "Sorotte syncplayintf returned a malformed settings acknowledgement".to_owned(),
                );
            }
            return;
        };
        if parsed.get("protocol").and_then(Value::as_str) != Some(SYNCPLAYINTF_PROTOCOL)
            || parsed.get("bridgeInstanceId").and_then(Value::as_str)
                != self.syncplayintf_bridge_instance_id.as_deref()
        ...
        let Some(pending_generation) = self.syncplayintf_pending_options_generation else {
            return;
        };
        ...
        if generation > pending_generation {
            self.syncplayintf_options_ack_error = Some(format!(
    ...
    pub(super) fn handle_syncplayintf_pong(&mut self, payload: Option<&str>) {
        ...
        if self.syncplayintf_pending_ping_nonce != Some(nonce) {
            return;
        }
        ...
        self.syncplayintf_bridge_instance_id = Some(bridge_instance_id.to_owned());
        self.syncplayintf_script_name = script_name.to_owned();
        self.syncplayintf_script_loaded = true;
        self.syncplayintf_pending_ping_nonce = None;
        self.syncplayintf_last_discovery_at = Some(Instant::now());
        if bridge_instance_changed {
    ...
    pub(super) fn handle_syncplayintf_lease_expired(&mut self, payload: Option<&str>) {
        if !self.syncplay_ui_settings.chat_input_enabled {
            return;
        }
        let Some(parsed) = payload.and_then(|payload| serde_json::from_str::<Value>(payload).ok())
    ...
    pub(super) fn handle_syncplayintf_chat_request(&mut self, payload: Option<&str>) {
