...
fn settings_without_osd_move() -> SyncplayUiSettings {
    SyncplayUiSettings {
        chat_move_osd: false,
...
fn bundled_bridge_degradation_preserves_core_playback_and_retry_after_simulated_release_recovers() {
    ...
    assert_eq!(adapter.take_pending_chat_request(), None);
    adapter.force_test_syncplayintf_heartbeat_due();
    assert_eq!(
        state.writes().len(),
    ...
    adapter
        .show_syncplay_message("bridge degraded", SyncplayOsdKind::Notification)
...
fn graceful_bridge_release_is_terminal_nonblocking_and_idempotent() {
    ...
    assert_eq!(writes[2]["command"][1], SYNCPLAYINTF_SCRIPT_NAME);
    ...
    assert_eq!(release_payload["protocol"], SYNCPLAYINTF_PROTOCOL);
...
fn graceful_bridge_release_restores_original_osd_placement_before_releasing_owner() {
    ...
    assert_eq!(
        writes[8]["command"][2], SYNCPLAYINTF_RELEASE_MESSAGE,
        "the input owner must be released after both OSD restoration attempts"
    );
    assert!(adapter.syncplay_osd_placement_restore().is_none());
}
...
fn disabled_bridge_configuration_never_disturbs_core_ipc() {
    let (transport, state) = fake_transport_with_reads(&[]);
    let mut adapter = MpvAdapter::with_test_transport(transport);
    let settings = SyncplayUiSettings {
...
fn missing_syncplay_bridge_loads_stable_resource_then_discovers_and_configures_it() {
    ...
    adapter
        .load_syncplayintf_script(resource_path)
        .expect("a missing stable target should be loaded and discovered");

    assert!(adapter.syncplayintf_script_loaded());
    assert!(adapter.syncplayintf_options_ready());
    let writes = parsed_writes(&state);
    ...
    assert_eq!(writes[3]["command"][1], SYNCPLAYINTF_SCRIPT_NAME);
...
fn newly_loaded_bridge_gets_a_bounded_registration_window_before_failure() {
    ...
    adapter
        .load_syncplayintf_script(std::path::Path::new(
...
fn existing_syncplay_bridge_is_pinged_and_reused_without_loading_a_duplicate() {
    ...
    assert_eq!(writes[0]["command"][1], SYNCPLAYINTF_SCRIPT_NAME);
    assert_eq!(writes[0]["command"][2], "sorotte_syncplayintf_ping");
    assert_eq!(writes[1]["command"][1], SYNCPLAYINTF_SCRIPT_NAME);
...
fn expired_input_lease_is_reacquired_with_a_fresh_acknowledged_generation() {
    ...
    assert!(
        adapter.syncplayintf_options_ready(),
...
fn rejected_runtime_settings_ack_degrades_with_typed_settings_failure() {
    ...
    adapter
        .configure_syncplay_ui_settings(baseline.clone())
        .expect("core settings should configure");
    assert_eq!(
        adapter.configure_bundled_sorotte_bridge(),
        SorotteBridgeHealth::Ready
    ...
    adapter
        .configure_syncplay_ui_settings(changed)
...
fn runtime_discovery_instance_change_reapplies_settings_and_emits_recovery_transitions() {
    ...
    adapter.force_test_syncplayintf_discovery_due();
...
fn bridge_targeted_output_failure_falls_back_and_bounded_rediscovery_degrades() {
    ...
    adapter
        .show_syncplay_chat_message("<alice> still visible")
...
fn syncplay_bridge_uses_typed_structured_settings_and_requires_an_exact_ack() {
    ...
    let mut adapter = MpvAdapter::with_test_transport(transport);
    adapter
        .configure_syncplay_ui_settings(SyncplayUiSettings {
    ...
    assert_eq!(payload["protocol"], SYNCPLAYINTF_PROTOCOL);
...
fn pending_syncplay_settings_retry_reuses_the_same_generation_until_acknowledged() {
    ...
    adapter
        .apply_pending_syncplayintf_options()
...
fn explicit_ipc_reattach_rediscovers_bridge_without_transferring_script_identity() {
    ...
    assert!(second_adapter.syncplayintf_options_ready());
...
#[test]
fn configure_syncplay_ui_settings_applies_osd_position_when_needed() {
    let (transport, state) = fake_transport_with_reads(&[
        r#"{"request_id":1,"error":"success","data":"top"}"#,
        r#"{"request_id":2,"error":"success","data":16}"#,
        r#"{"request_id":3,"error":"success"}"#,
...
#[test]
fn configure_syncplay_ui_settings_restores_osd_position_when_move_is_disabled() {
    let (transport, state) = fake_transport_with_reads(&[
        r#"{"request_id":1,"error":"success","data":"top"}"#,
        r#"{"request_id":2,"error":"success","data":16}"#,
        r#"{"request_id":3,"error":"success"}"#,
...
fn transferred_osd_restore_state_survives_an_explicit_ipc_reattach() {
    ...
    let restore = first_adapter
        .syncplay_osd_placement_restore()
    ...
    reattached_adapter.set_syncplay_osd_placement_restore(Some(restore));
    reattached_adapter
        .configure_syncplay_ui_settings(settings_without_osd_move())
...
#[test]
fn configure_syncplay_ui_settings_skips_osd_position_when_disabled() {
    let (transport, state) = fake_transport_with_reads(&[]);
    let mut adapter = MpvAdapter::with_test_transport(transport);
...
#[test]
fn show_syncplay_message_uses_notification_timeout_for_show_text() {
...
#[test]
fn show_syncplay_message_uses_alert_timeout_for_show_text() {
    let (transport, state) = fake_transport_with_reads(&[r#"{"request_id":1,"error":"success"}"#]);
    let mut adapter = MpvAdapter::with_test_transport(transport);
    adapter
        .configure_syncplay_ui_settings(SyncplayUiSettings {
...
#[test]
fn show_syncplay_chat_message_uses_chat_timeout_when_bridge_is_unavailable() {
    let (transport, state) = fake_transport_with_reads(&[r#"{"request_id":1,"error":"success"}"#]);
    let mut adapter = MpvAdapter::with_test_transport(transport);
    adapter
        .configure_syncplay_ui_settings(SyncplayUiSettings {
...
#[test]
fn show_syncplay_chat_message_uses_notification_timeout_when_chat_output_is_disabled() {
    let (transport, state) = fake_transport_with_reads(&[r#"{"request_id":1,"error":"success"}"#]);
    let mut adapter = MpvAdapter::with_test_transport(transport);
    adapter
        .configure_syncplay_ui_settings(SyncplayUiSettings {
