...
use crate::constants::{
    ...
    SOROTTE_NETWORK_OPTIONS_CLIENT_MESSAGE_TRANSITION_RESULT, SYNCPLAYINTF_CLIENT_MESSAGE_CHAT,
...
fn sorotte_client_message_context(message_name: &str) -> Option<SanitizationContext> {
    match message_name {
        SYNCPLAYINTF_CLIENT_MESSAGE_CHAT => Some(SanitizationContext::Opaque),
...
mod tests {
    ...
    fn recognized_sorotte_client_messages_keep_routing_and_control_payload_shape() {
        ...
        let sanitized_chat = sanitize_json(json!({
            "event": MPV_EVENT_CLIENT_MESSAGE,
            "args": [SYNCPLAYINTF_CLIENT_MESSAGE_CHAT, private_chat],
        }));
        assert_eq!(
            sanitized_chat["args"][0], SYNCPLAYINTF_CLIENT_MESSAGE_CHAT,
