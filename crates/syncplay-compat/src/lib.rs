...
mod tests {
    ...
    const PLAYLIST_ROOM_SWITCH_SNAPSHOT_THEN_DESTINATION_UPDATE_ORDERING_SCENARIO: &str =
        "server_runtime_playlist_room_switch_snapshot_then_destination_update_ordering.jsonl";
    const PLAYLIST_ROOM_SWITCH_SNAPSHOT_THEN_OLD_ROOM_UPDATE_ORDERING_SCENARIO: &str =
        "server_runtime_playlist_room_switch_snapshot_then_old_room_update_ordering.jsonl";
    const PLAYLIST_ROOM_SWITCH_SNAPSHOT_THEN_OLD_THEN_DESTINATION_UPDATE_ORDERING_SCENARIO: &str = "server_runtime_playlist_room_switch_snapshot_then_old_then_destination_update_ordering.jsonl";
    const PLAYLIST_ROOM_SWITCH_SNAPSHOT_THEN_DESTINATION_THEN_OLD_UPDATE_ORDERING_SCENARIO: &str = "server_runtime_playlist_room_switch_snapshot_then_destination_then_old_update_ordering.jsonl";
    ...
    const CHAT_ROOM_SWITCH_PEER_TRANSITION_SCOPING_SCENARIO: &str =
        "server_runtime_chat_room_switch_peer_transition_scoping.jsonl";
    const CHAT_ROOM_SWITCH_OBJECT_PAYLOAD_SCOPING_SCENARIO: &str =
        "server_runtime_chat_room_switch_object_payload_scoping.jsonl";
    const CHAT_DOUBLE_ROOM_SWITCH_SCOPING_SCENARIO: &str =
    ...
    fn overwrite_file_until_modified_time_changes(
        path: &Path,
        contents: &str,
    ) -> Result<(), InteropError> {
            ...
            .filter_map(|outbound| {
                ...
                payload
                    ...
                    .filter(|playlist_change| {
                        playlist_change.files == vec!["room2-ep2.mkv".to_owned()]
                    })
            ...
            .filter_map(|outbound| {
                ...
                payload
                    ...
                    .filter(|playlist_change| {
                        playlist_change.user.as_deref() == Some("alice")
                            && playlist_change.files
                                == vec!["room1-after-bob-switch.mkv".to_owned()]
                    })
                    .map(|_| outbound.client_id.clone())
            })
            .collect();
                ...
                payload
                    ...
                    .filter(|playlist_change| {
                        playlist_change.user.as_deref() == Some("bob")
                            && playlist_change.files
                                == vec!["room2-bob-after-switch.mkv".to_owned()]
                    })
                    .map(|_| outbound.client_id.clone())
            })
            .collect();
                ...
                match chat_message.chat {
                    ChatPayload::Message(message)
                        if message.username == "bob"
                            && message.message == "before move from bob" =>
                    {
                        Some(outbound.client_id.clone())
                    }
                    _ => None,
        ...
        let step6_chat_recipients: Vec<_> = step6
            ...
            .filter_map(|outbound| {
                ...
                match chat_message.chat {
                    ChatPayload::Message(message)
                        if message.username == "bob"
                            && message.message == "after move from bob" =>
                    {
                        Some(outbound.client_id.clone())
                    }
                    _ => None,
                ...
                match chat_message.chat {
                    ChatPayload::Message(message)
                        if message.username == "alice"
                            && message.message == "room1 after bob left" =>
                    {
                        Some(outbound.client_id.clone())
                    }
                    _ => None,
        ...
        let step7_chat_recipients: Vec<_> = step7
            ...
            .filter_map(|outbound| {
                ...
                match chat_message.chat {
                    ChatPayload::Message(message)
                        if message.username == "alice"
                            && message.message == "room1 still has bob" =>
                    {
                        Some(outbound.client_id.clone())
                    }
                    _ => None,
    ...
    fn legacy_server_live_tls_rotation_invalidates_subsequent_send() {
        ...
        let result = run_legacy_server_tls_rotation_invalidates_subsequent_send_with_cert_path(
            &tls_cert_path,
        );
        let _ = fs::remove_dir_all(&tls_cert_path);
