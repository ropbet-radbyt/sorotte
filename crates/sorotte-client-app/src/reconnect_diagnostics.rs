...
fn localized_reconnect_correction_metrics_prefix(language: Option<&str>) -> &'static str {
    match language {
...
pub fn reconnect_correction_metrics_delta_message_localized(
    previous: Option<&ReconnectStateRestoreCorrectionMetrics>,
    current: &ReconnectStateRestoreCorrectionMetrics,
    language: Option<&str>,
) -> Option<String> {
    ...
    if fields.is_empty() {
        ...
        Some(format!(
            "{}: {}",
            localized_reconnect_correction_metrics_prefix(language),
            fields.join(" ")
        ))
    }
}
...
fn localized_reconnect_correction_state_prefix(language: Option<&str>) -> &'static str {
    match language {
...
pub fn reconnect_correction_state_snapshot_message_localized(
    snapshot: &ReconnectStateRestoreCorrectionStateSnapshot,
    language: Option<&str>,
) -> String {
    if language.is_none() {
    ...
    format!(
        ...
        localized_reconnect_correction_state_prefix(language),
        snapshot.validation_pending,
        reconnect_correction_policy_mode_label(snapshot.effective_policy_mode),
        snapshot.position_tolerance_seconds,
        snapshot.retry_attempts,
...
fn localized_reconnect_correction_alert_prefix(language: Option<&str>) -> &'static str {
    match language {
...
fn reconnect_correction_metric_delta_alert_text_localized(
    metric: &str,
    delta: u64,
    total: u64,
    threshold: u64,
    ...
    format!(
        ...
        localized_reconnect_correction_alert_prefix(language)
    )
}
...
fn reconnect_correction_state_threshold_alert_text_localized(
    metric: &str,
    value: u32,
    threshold: u32,
    language: Option<&str>,
...
pub fn next_reconnect_correction_diagnostic_lines(
    ...
    if state.last_snapshot.as_ref() != Some(&snapshot) {
        let message = match format {
            ReconnectCorrectionDiagnosticsFormat::Text => {
                reconnect_correction_state_snapshot_message_localized(&snapshot, language)
            }
            ReconnectCorrectionDiagnosticsFormat::Json => {
    ...
    let state_alerts = match format {
        ReconnectCorrectionDiagnosticsFormat::Text => {
            reconnect_correction_state_threshold_alert_lines_localized(
...
mod tests {
    ...
    fn next_reconnect_correction_diagnostic_lines_emits_metrics_and_snapshot_once() {
        ...
        let lines = next_reconnect_correction_diagnostic_lines(
            &mut state,
    ...
    fn next_reconnect_correction_diagnostic_lines_honors_threshold_alerts_and_dedupes_snapshot() {
        ...
        let first = next_reconnect_correction_diagnostic_lines(
            &mut state,
            ReconnectStateRestoreCorrectionMetrics {
        ...
        let second = next_reconnect_correction_diagnostic_lines(
            &mut state,
            ReconnectStateRestoreCorrectionMetrics {
                correction_action_failures: 2,
                ..ReconnectStateRestoreCorrectionMetrics::default()
