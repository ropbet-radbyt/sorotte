use crate::stored_settings::{
    StoredClientSettings, autoplay_threshold_override_setting_value, privacy_mode_syncplay_name,
...
use crate::syncplay_ini_values::{
    ...
    format_serialized_string_list,
};

use super::helpers::{
    format_ini_bool, format_ini_non_negative_f64, remove_ini_value, upsert_ini_value,
};
...
fn upsert_sorotte_ini_stored_client_settings_with_plex_identity_clear(
    ...
    if let Some(host) = settings.host.as_deref() {
        upsert_ini_value(&mut lines, "server_data", "host", host);
    }
    if let Some(port) = settings.port {
        upsert_ini_value(&mut lines, "server_data", "port", &port.to_string());
    }
    ...
    {
        upsert_ini_value(&mut lines, "server_data", "password", server_password);
    }
    if let Some(tls_policy) = settings.tls_policy.as_deref() {
        upsert_ini_value(&mut lines, "server_data", "tlsPolicy", tls_policy);
    }
    if let Some(username) = settings.username.as_deref() {
        upsert_ini_value(&mut lines, "client_settings", "name", username);
    }
    if let Some(room) = settings.room.as_deref() {
        upsert_ini_value(&mut lines, "client_settings", "room", room);
    }
    if let Some(room_list) = settings.room_list.as_ref() {
        let serialized = format_serialized_string_list(room_list);
        upsert_ini_value(&mut lines, "client_settings", "roomList", &serialized);
    }
    if let Some(player_path) = settings.player_path.as_deref() {
        upsert_ini_value(&mut lines, "client_settings", "playerPath", player_path);
    }
    if let Some(per_player_arguments) = settings.per_player_arguments.as_ref() {
        let serialized = format_serialized_per_player_arguments_map(per_player_arguments);
        upsert_ini_value(
            &mut lines,
            "client_settings",
            "perPlayerArguments",
    ...
    if let Some(media_search_directories) = settings.media_search_directories.as_ref() {
        let serialized = format_serialized_string_list(media_search_directories);
        upsert_ini_value(
            &mut lines,
            "client_settings",
    ...
    if let Some(public_servers) = settings.public_servers.as_ref() {
        let serialized = format_serialized_public_servers_list(public_servers);
        upsert_ini_value(&mut lines, "client_settings", "publicServers", &serialized);
    }
    ...
    if let Some(value) = settings.plex_plugin_enabled {
        upsert_ini_value(&mut lines, "plugins", "plexEnabled", format_ini_bool(value));
    }
    ...
    if let Some(value) = settings.plex_sync_enabled {
        upsert_ini_value(&mut lines, "plex", "syncEnabled", format_ini_bool(value));
    }
    ...
    {
        upsert_ini_value(&mut lines, "plex", "userToken", value);
    }
    if let Some(value) = settings.plex_selected_server_id.as_deref() {
        upsert_ini_value(&mut lines, "plex", "selectedServerId", value);
    }
    if let Some(value) = settings.plex_selected_server_url.as_deref() {
        upsert_ini_value(&mut lines, "plex", "selectedServerUrl", value);
    }
    if let Some(value) = settings
        ...
        .map(|token| token.expose_secret())
    {
        upsert_ini_value(&mut lines, "plex", "selectedServerToken", value);
    }
    if let Some(value) = settings.folder_search_first_file_timeout_seconds
        && let Some(formatted) = format_ini_non_negative_f64(value)
    {
        upsert_ini_value(
            &mut lines,
            "client_settings",
    ...
    if let Some(trusted_domains) = settings.trusted_domains.as_ref() {
        let serialized = format_serialized_string_list(trusted_domains);
        upsert_ini_value(&mut lines, "client_settings", "trustedDomains", &serialized);
    }
    ...
    if let Some(value) = settings.rewind_threshold_seconds
        && let Some(formatted) = format_ini_non_negative_f64(value)
    {
        upsert_ini_value(&mut lines, "client_settings", "rewindThreshold", &formatted);
    }
    ...
    if let Some(unpause_action) = settings.unpause_action.as_ref() {
        upsert_ini_value(
            ...
            "client_settings",
            "unpauseAction",
            unpause_action_mode_syncplay_name(unpause_action.clone()),
        );
    }
    if let Some(autoplay_min_users) = settings.autoplay_min_users.as_ref() {
        upsert_ini_value(
            ...
            "client_settings",
            "autoplayMinUsers",
            &autoplay_threshold_override_setting_value(autoplay_min_users),
        );
    }
    if let Some(mode) = settings.filename_privacy_mode {
        upsert_ini_value(
            ...
            "client_settings",
            "filenamePrivacyMode",
            privacy_mode_syncplay_name(mode),
        );
    }
    ...
    if let Some(language) = settings.language.as_deref() {
        upsert_ini_value(&mut lines, "general", "language", language);
    }
    ...
    if let Some(value) = settings.update_channel.as_deref() {
        upsert_ini_value(&mut lines, "general", "updateChannel", value);
    }
    if let Some(value) = settings.last_checked_for_updates.as_deref() {
        upsert_ini_value(&mut lines, "general", "lastCheckedForUpdates", value);
    }
    ...
    if let Some(value) = settings.show_osd {
        upsert_ini_value(&mut lines, "gui", "showOSD", format_ini_bool(value));
    }
    ...
    if let Some(value) = settings.chat_input_font_family.as_deref() {
        upsert_ini_value(&mut lines, "gui", "chatInputFontFamily", value);
    }
    ...
    if let Some(value) = settings.chat_input_font_weight {
        upsert_ini_value(&mut lines, "gui", "chatInputFontWeight", &value.to_string());
    }
    if let Some(value) = settings.chat_input_font_color.as_deref() {
        upsert_ini_value(&mut lines, "gui", "chatInputFontColor", value);
    }
    if let Some(value) = settings.chat_input_position.as_deref() {
        upsert_ini_value(&mut lines, "gui", "chatInputPosition", value);
    }
    if let Some(value) = settings.chat_direct_input {
        upsert_ini_value(&mut lines, "gui", "chatDirectInput", format_ini_bool(value));
    }
    ...
    if let Some(value) = settings.chat_output_font_family.as_deref() {
        upsert_ini_value(&mut lines, "gui", "chatOutputFontFamily", value);
    }
    ...
    if let Some(value) = settings.chat_output_mode.as_deref() {
        upsert_ini_value(&mut lines, "gui", "chatOutputMode", value);
    }
    if let Some(value) = settings.chat_move_osd {
        upsert_ini_value(&mut lines, "gui", "chatMoveOSD", format_ini_bool(value));
    }
    if let Some(value) = settings.chat_max_lines {
        upsert_ini_value(&mut lines, "gui", "chatMaxLines", &value.to_string());
    }
    if let Some(value) = settings.chat_top_margin {
        upsert_ini_value(&mut lines, "gui", "chatTopMargin", &value.to_string());
    }
    if let Some(value) = settings.chat_left_margin {
        upsert_ini_value(&mut lines, "gui", "chatLeftMargin", &value.to_string());
    }
    if let Some(value) = settings.chat_bottom_margin {
        upsert_ini_value(&mut lines, "gui", "chatBottomMargin", &value.to_string());
    }
    if let Some(value) = settings.chat_osd_margin {
        upsert_ini_value(&mut lines, "gui", "chatOSDMargin", &value.to_string());
    }
    if let Some(value) = settings.notification_timeout_seconds {
        upsert_ini_value(&mut lines, "gui", "notificationTimeout", &value.to_string());
    }
    if let Some(value) = settings.alert_timeout_seconds {
        upsert_ini_value(&mut lines, "gui", "alertTimeout", &value.to_string());
    }
    if let Some(value) = settings.chat_timeout_seconds {
        upsert_ini_value(&mut lines, "gui", "chatTimeout", &value.to_string());
    }
    ...
    if let Some(value) = settings.show_same_room_osd {
        upsert_ini_value(&mut lines, "gui", "showSameRoomOSD", format_ini_bool(value));
    }
    if let Some(value) = settings.show_osd_warnings {
        upsert_ini_value(&mut lines, "gui", "showOSDWarnings", format_ini_bool(value));
    }
    if let Some(value) = settings.show_slowdown_osd {
        upsert_ini_value(&mut lines, "gui", "showSlowdownOSD", format_ini_bool(value));
    }
    ...
    if let Some(value) = settings.show_contact_info {
        upsert_ini_value(&mut lines, "gui", "showContactInfo", format_ini_bool(value));
    }
...
fn upsert_optional_streaming_settings(lines: &mut Vec<String>, settings: &StoredClientSettings) {
    for (key, value) in [
        (
        ...
        if let Some(value) = value {
            upsert_ini_value(lines, "client_settings", key, value);
        }
    }
    ...
    for (key, value) in [
        ...
        if let Some(value) = value
            && let Some(rendered) = format_ini_non_negative_f64(value)
        {
            upsert_ini_value(lines, "client_settings", key, &rendered);
        }
    }
    ...
    for (key, value) in [
        ...
        if let Some(value) = value {
            upsert_ini_value(lines, "client_settings", key, &value.to_string());
        }
    }
    ...
    for (key, value) in [
        ...
        if let Some(value) = value {
            upsert_ini_value(lines, "client_settings", key, format_ini_bool(value));
        }
    }
}
