...
fn localized_user_joined_room_phrase(language: Option<&str>) -> &'static str {
    match language {
...
fn localized_user_playing_phrase(language: Option<&str>) -> &'static str {
    match language {
        Some("de") => "spielt",
...
fn localized_user_playing_file_phrase(language: Option<&str>) -> &'static str {
    match language {
...
fn localized_user_room_addendum_phrase(language: Option<&str>) -> &'static str {
    match language {
        Some("de") => "in Raum",
        Some("es") => "en la sala",
        Some("eo") => "en cxambro",
...
fn localized_user_left_phrase(language: Option<&str>) -> &'static str {
    match language {
...
pub fn user_change_notification_message_localized(
    ...
    match notification {
        UserChangeNotification::Joined { username, room, .. } => format!(
            "{username} {}: '{room}'",
            localized_user_joined_room_phrase(language)
        ),
        UserChangeNotification::Playing {
            username,
            room,
            ...
            Some(file_name) => {
                let mut message = if let Some(duration_seconds) = file_duration {
                    format!(
                        "{username} {} '{file_name}' ({})",
                        localized_user_playing_phrase(language),
                        format_duration(*duration_seconds)
                    )
                } else {
                    format!(
                ...
                if *include_room_addendum {
                    message.push_str(&format!(
                        " {}: '{room}'",
                        localized_user_room_addendum_phrase(language)
                    ));
                }
                message
            }
            None if *include_room_addendum => format!(
                "{username} {} {}: '{room}'",
                localized_user_playing_file_phrase(language),
                localized_user_room_addendum_phrase(language)
            ),
            None => format!(
                "{username} {}",
        ...
        UserChangeNotification::Left { username, .. } => {
            format!("{username} {}", localized_user_left_phrase(language))
        }
    }
}
