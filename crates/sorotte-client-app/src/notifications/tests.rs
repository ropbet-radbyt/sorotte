...
use super::{
    ...
    localized_file_difference_summary, next_file_difference_notification_summary,
...
fn file_difference_helpers_format_localize_and_dedupe() {
    ...
    assert_eq!(
        localized_file_difference_summary("filename, duration", Some("de")),
        "Dateiname, Dauer"
    );
    assert_eq!(
        localized_file_difference_notification_line("filename, duration", Some("de")),
        "Dateiunterschiede: Dateiname, Dauer"
    );

    let mut state = FileDifferenceNotificationState::default();
    let first = next_file_difference_notification_summary(
        &mut state,
    ...
    let second = next_file_difference_notification_summary(
        &mut state,
        Some(FileDifferenceSummary {
            filename: true,
            filesize: false,
    ...
    let cleared = next_file_difference_notification_summary(&mut state, None);
