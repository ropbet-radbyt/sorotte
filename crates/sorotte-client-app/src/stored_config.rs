...
pub fn stored_client_settings_config_plan(
    settings: &StoredClientSettings,
...
mod tests {
    ...
    use super::{
        ...
        stored_client_settings_config_plan, stored_client_settings_runtime_snapshot,
    };

    fn stored_settings_with_every_runtime_override() -> StoredClientSettings {
    ...
    fn legacy_config_plan_does_not_apply_invalid_embedded_or_explicit_zero_ports() {
        let embedded_settings = StoredClientSettings {
            host: Some("example.org:0".to_owned()),
            ..StoredClientSettings::default()
        };
        let embedded_snapshot = stored_client_settings_runtime_snapshot(&embedded_settings);
        let embedded_plan = stored_client_settings_config_plan(
            &embedded_settings,
            &StoredClientSettingsEnvPresence::default(),
        );
        ...
        let explicit_settings = StoredClientSettings {
            host: Some("example.org:8123".to_owned()),
            port: Some(0),
            ..StoredClientSettings::default()
        };
        let explicit_snapshot = stored_client_settings_runtime_snapshot(&explicit_settings);
        let explicit_plan = stored_client_settings_config_plan(
            &explicit_settings,
            &StoredClientSettingsEnvPresence::default(),
        );
    ...
    #[test]
    fn stored_client_settings_config_plan_applies_only_values_not_shadowed_by_env() {
        let plan = stored_client_settings_config_plan(
            &StoredClientSettings {
                host: Some("stored.example".to_owned()),
                port: Some(8999),
    ...
    #[test]
    fn stored_client_settings_config_plan_carries_runtime_fallbacks_forward() {
        let plan = stored_client_settings_config_plan(
            &StoredClientSettings {
                room_list: Some(vec![" ".to_owned(), "+room:ABCDEF123456:AB-123".to_owned()]),
                public_servers: Some(vec![("Public".to_owned(), "example.org:8999".to_owned())]),
