...
const PLAYLIST_INVALID_INDEX_ERROR: &str = "Invalid playlist index";
const QUEUE_MISSING_FILE_ERROR: &str = "No file/url given";
const UNKNOWN_COMMAND_MESSAGE: &str = "Unrecognized command";
const SYNCPLAY_PROJECT_URL: &str = "https://syncplay.pl/";

pub fn localized_local_input_error_message(
    ...
    match (error_kind, language) {
        ...
        (LocalInputCommandErrorKind::PlaylistInvalidIndex, _) => PLAYLIST_INVALID_INDEX_ERROR,
        ...
        (LocalInputCommandErrorKind::QueueMissingFile, _) => QUEUE_MISSING_FILE_ERROR,
    }
}

fn localized_error_prefix(language: Option<&str>) -> &'static str {
    match language {
        Some("de") => "FEHLER",
        Some("es") => "ERROR",
        Some("eo") => "ERARO",
...
pub fn local_input_error_output_line(
    error_kind: LocalInputCommandErrorKind,
    language: Option<&str>,
) -> String {
    format!(
        "{}:\t{}",
        localized_error_prefix(language),
        localized_local_input_error_message(error_kind, language)
    )
}

pub(crate) fn localized_unknown_command_message(language: Option<&str>) -> &'static str {
    match language {
        ...
        _ => UNKNOWN_COMMAND_MESSAGE,
    }
}

fn localized_local_command_help_heading(language: Option<&str>) -> &'static str {
    match language {
...
fn local_command_help_command_lines() -> &'static [&'static str] {
    &[
        "\tr [name] - change room",
        "\tl - show user list",
        "\tu - undo last seek",
        ...
        "\tready [name] - sets user as ready",
        "\tnot-ready [name] - sets user as not ready",
...
fn localized_local_command_help_command_lines(language: Option<&str>) -> &'static [&'static str] {
    match language {
        Some("de") => &[
        ...
        _ => local_command_help_command_lines(),
    }
}

pub(crate) fn local_command_help_lines(language: Option<&str>) -> Vec<String> {
    let command_lines = localized_local_command_help_command_lines(language);
    let mut lines = Vec::with_capacity(command_lines.len() + 4);
    lines.push(localized_local_command_help_heading(language).to_owned());
...
fn localized_syncplay_version_prefix(language: Option<&str>) -> &'static str {
    match language {
...
fn localized_more_info_prefix(language: Option<&str>) -> &'static str {
    match language {
...
pub(crate) fn local_command_help_footer_lines(
    language: Option<&str>,
    version: &str,
) -> [String; 2] {
    [
        format!("{}: {version}", localized_syncplay_version_prefix(language)),
        format!(
            "{}: {SYNCPLAY_PROJECT_URL}",
            localized_more_info_prefix(language)
        ),
    ]
}

pub fn render_local_input_display_lines(
    ...
    match dispatch {
        ...
        PlannedLocalInputDispatch::EmitUnknownCommandHelp => {
            ...
            lines.push(localized_unknown_command_message(language).to_owned());
            lines.extend(local_command_help_lines(language));
            lines.extend(local_command_help_footer_lines(language, version));
            Some(lines)
        }
        PlannedLocalInputDispatch::EmitHelp => {
            let mut lines = local_command_help_lines(language);
            lines.extend(local_command_help_footer_lines(language, version));
            Some(lines)
        }
        PlannedLocalInputDispatch::EmitError(error_kind) => {
            Some(vec![local_input_error_output_line(*error_kind, language)])
        }
        PlannedLocalInputDispatch::EmitPlaylist => {
            Some(vec![playlist_listing_message_localized(session, language)])
        }
    }
}

pub fn localized_current_offset_message(offset_seconds: f64, language: Option<&str>) -> String {
    match language {
...
fn localized_playlist_empty_message(language: Option<&str>) -> &'static str {
    match language {
        ...
        _ => PLAYLIST_EMPTY_MESSAGE,
    }
}

pub fn playlist_listing_message(session: &ClientSession) -> String {
    let Some(playlist) = session.current_room_playlist() else {
        return PLAYLIST_EMPTY_MESSAGE.to_owned();
    };
...
pub fn playlist_listing_message_localized(
    session: &ClientSession,
    language: Option<&str>,
) -> String {
    let Some(playlist) = session.current_room_playlist() else {
        return localized_playlist_empty_message(language).to_owned();
    };
    if playlist.files.is_empty() {
        return localized_playlist_empty_message(language).to_owned();
    }
