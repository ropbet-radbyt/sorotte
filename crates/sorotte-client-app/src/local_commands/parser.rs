...
fn parse_create_command(input: &str) -> Option<Option<String>> {
...
fn parse_room_command(input: &str) -> Option<Option<LocalInputCommand>> {
    for alias in ["room", "r"] {
        if input == alias {
            return Some(Some(LocalInputCommand::SetRoomWithDefaultFallback));
        }
...
fn parse_time_seconds_with_component_limits(
    value: &str,
...
pub fn parse_seek_time_seconds(value: &str) -> Option<f64> {
    parse_time_seconds_with_component_limits(value, 4, 6)
}

fn parse_offset_time_seconds(value: &str) -> Option<f64> {
    parse_time_seconds_with_component_limits(value, 9, 9)
}
...
fn parse_seek_parameter(parameter: &str) -> Option<LocalInputCommand> {
    ...
    if let Some(value) = parameter.strip_prefix('+') {
        let seconds = parse_seek_time_seconds(value)?;
    ...
    let seconds = parse_seek_time_seconds(parameter)?;
    Some(LocalInputCommand::SeekAbsolute(seconds))
}

fn parse_offset_parameter(parameter: &str) -> Option<LocalOffsetCommand> {
    if parameter.is_empty() {
        return None;
    }

    if let Some(value) = parameter.strip_prefix('+') {
        let seconds = parse_offset_time_seconds(value)?;
    ...
    let seconds = parse_offset_time_seconds(parameter)?;
    Some(LocalOffsetCommand::Absolute(seconds))
}

fn parse_offset_input(input: &str) -> Option<LocalInputCommand> {
    ...
    let offset_command = parse_offset_parameter(parameter)?;
    Some(LocalInputCommand::SetUserOffset(offset_command))
}

fn parse_seek_input(input: &str) -> Option<LocalInputCommand> {
    if input.is_empty() {
        return None;
    }
...
fn parse_playlist_index_parameter(parameter: &str) -> Option<i64> {
...
fn matches_local_command_alias(input: &str, aliases: &[&str]) -> bool {
...
fn is_known_local_command_token(token: &str) -> bool {
    matches!(
        token,
        "help"
            | "h"
...
pub fn parse_local_input_command(input: &str) -> Option<LocalInputCommand> {
    ...
    if matches_local_command_alias(trimmed, &["help", "h", "?", "/?", "\\?"]) {
        return Some(LocalInputCommand::ShowHelp);
    }
    if matches_local_command_alias(trimmed, &["keep-waiting"]) {
        return Some(LocalInputCommand::KeepWaitingForSeekPreparation);
    }
    if matches_local_command_alias(trimmed, &["join-nearest-buffered-position"]) {
        return Some(LocalInputCommand::JoinNearestBufferedSeekPreparation);
    }
    if matches_local_command_alias(trimmed, &["cancel-and-remain"]) {
        return Some(LocalInputCommand::CancelSeekPreparation);
    }
    if matches_local_command_alias(trimmed, &["undoplaylist"]) {
        return Some(LocalInputCommand::UndoPlaylistChange);
    }
    if matches_local_command_alias(trimmed, &["shuffleremainingplaylist"]) {
        return Some(LocalInputCommand::ShuffleRemainingPlaylist);
    }
    if matches_local_command_alias(trimmed, &["shuffleentireplaylist"]) {
        return Some(LocalInputCommand::ShuffleEntirePlaylist);
    }
    if matches_local_command_alias(trimmed, &["undo", "u", "revert"]) {
        return Some(LocalInputCommand::UndoSeek);
    }
    if matches_local_command_alias(trimmed, &["list", "l", "users"]) {
        return Some(LocalInputCommand::RequestUserList);
    }
    if matches_local_command_alias(trimmed, &["playlist", "ql", "pl"]) {
    ...
    {
        return parse_playlist_index_parameter(index)
    ...
    if matches_local_command_alias(trimmed, &["next", "qn"]) {
        return Some(LocalInputCommand::NextPlaylistItem);
    }
    if let Some(command) = parse_queue_command(input, &["queueandselect", "qas"], true) {
        return Some(command);
    }
    if let Some(command) = parse_queue_command(input, &["queue", "qa", "add"], false) {
        return Some(command);
    }
    if let Some(index) = trimmed
        .strip_prefix("delete ")
    ...
    if let Some(command) = parse_user_ready_command(input, &["setready", "ready", "sr"], true) {
        return Some(command);
    }
    if let Some(command) =
        parse_user_ready_command(input, &["setnotready", "not-ready", "sn", "snr"], false)
    {
        return Some(command);
    }
    if let Some(room_name) = parse_create_command(input) {
    ...
    if matches_local_command_alias(trimmed, &["play"]) {
        return Some(LocalInputCommand::Play);
    }
    if matches_local_command_alias(trimmed, &["pause"]) {
        return Some(LocalInputCommand::Pause);
    }
    if matches_local_command_alias(trimmed, &["p"]) {
        return Some(LocalInputCommand::TogglePause);
    }
    if let Some(room_command) = parse_room_command(input) {
        return room_command;
    }
    if matches_local_command_alias(trimmed, &["t", "toggle"]) {
        return Some(LocalInputCommand::ToggleReady);
    }
    if let Some(command) = parse_offset_input(input) {
        return Some(command);
    }
    ...
    if let Some(command) = parse_seek_input(input) {
        return Some(command);
    }
    ...
    if is_known_local_command_token(command_token) {
        return Some(LocalInputCommand::ShowUnknownCommandHelp);
    }
