...
fn controlled_room_base_name_strips_managed_suffix() {
    ...
    assert_eq!(
        controlled_room_base_name("+room:SHORT"),
        "+room:SHORT".to_owned()
    );
}
...
#[test]
fn resolved_local_user_offset_seconds_applies_all_modes() {
    assert_eq!(
        resolved_local_user_offset_seconds(5.0, 100.0, &LocalOffsetCommand::Absolute(12.0),),
        12.0
    );
    assert_eq!(
        resolved_local_user_offset_seconds(5.0, 100.0, &LocalOffsetCommand::Relative(3.0),),
        8.0
    );
    assert_eq!(
        resolved_local_user_offset_seconds(
...
#[test]
fn playlist_index_in_bounds_matches_current_room_playlist() {
    ...
    assert!(!playlist_index_in_bounds(&session, 2));
    assert!(!playlist_index_in_bounds(&session, -1));
}

#[test]
fn playlist_listing_message_formats_entries_and_empty_states() {
    let mut session = sorotte_client_core::ClientSession::default();
    session
        .apply_hello_json(
    ...
    assert_eq!(
        playlist_listing_message(&session),
...
#[test]
fn playlist_listing_message_localized_uses_localized_empty_message() {
    let mut session = sorotte_client_core::ClientSession::default();
    session
        .apply_hello_json(
            r#"{"Hello":{"username":"alice","room":{"name":"room1"},"version":"1.7.5"}}"#,
        ...
        .expect("hello should apply");
    assert_eq!(
        playlist_listing_message_localized(&session, Some("fr")),
        "La playlist est actuellement vide."
    );
    assert_eq!(
        playlist_listing_message_localized(&session, Some("de")),
        "Playlist ist derzeit leer."
    );
}

#[test]
fn localized_local_input_error_message_localizes_known_messages() {
    assert_eq!(
        localized_local_input_error_message(
    ...
    assert_eq!(
        localized_local_input_error_message(LocalInputCommandErrorKind::QueueMissingFile, None,),
        "No file/url given"
    );
}

#[test]
fn local_input_error_output_line_formats_prefix_and_message() {
    assert_eq!(
        local_input_error_output_line(LocalInputCommandErrorKind::PlaylistInvalidIndex, Some("de"),),
        "FEHLER:\tUngueltiger Playlist-Index"
    );
    assert_eq!(
        local_input_error_output_line(LocalInputCommandErrorKind::QueueMissingFile, None,),
        "ERROR:\tNo file/url given"
    );
}

#[test]
fn local_command_help_lines_include_expected_entries() {
    let lines = local_command_help_lines(None);
    assert_eq!(
...
#[test]
fn local_command_help_lines_localize_heading_and_body() {
    let lines = local_command_help_lines(Some("fr"));
    assert_eq!(
        lines.first().map(String::as_str),
...
#[test]
fn local_command_help_footer_lines_include_expected_entries() {
    let lines = local_command_help_footer_lines(Some("de"), "1.7.5");
    assert_eq!(lines[0], "Sorotte-Version: 1.7.5");
    assert_eq!(lines[1], "Mehr Informationen unter: https://syncplay.pl/");
}

#[test]
fn localized_unknown_command_message_uses_selected_language() {
    assert_eq!(
        localized_unknown_command_message(Some("es")),
        "Comando no reconocido"
    );
    assert_eq!(
        localized_unknown_command_message(None),
        "Unrecognized command"
    );
}

#[test]
fn localized_current_offset_message_localizes_user_visible_message() {
    assert_eq!(
        localized_current_offset_message(2.5, Some("pt_BR")),
        "Deslocamento atual: 2.5 segundos"
    );
    assert_eq!(
        localized_current_offset_message(-1.0, None),
        "Current offset: -1 seconds"
    );
}

#[test]
fn plan_local_offset_runtime_dispatch_emits_seek_and_status_line() {
...
#[test]
fn render_local_input_display_lines_renders_unknown_help_and_playlist() {
    let mut session = sorotte_client_core::ClientSession::default();
    session
        .apply_hello_json(
            r#"{"Hello":{"username":"alice","room":{"name":"room1"},"version":"1.7.5"}}"#,
    ...
    let unknown_lines = render_local_input_display_lines(
    ...
    let playlist_lines = render_local_input_display_lines(
...
#[test]
fn plan_local_playlist_runtime_dispatch_handles_valid_and_invalid_indices() {
    let mut session = sorotte_client_core::ClientSession::default();
    session
        .apply_message_json(
            r#"{"Hello":{"username":"alice","room":{"name":"room1"},"version":"1.2.255"}}"#,
    ...
    let invalid_dispatch = plan_local_playlist_select_runtime_dispatch(&session, 5, Some("fr"));
    assert_eq!(
    ...
    let valid_dispatch = plan_local_playlist_delete_runtime_dispatch(&session, 1, Some("fr"));
...
#[test]
fn plan_local_runtime_dispatch_promotes_special_cases() {
    let offset_dispatch = plan_local_runtime_dispatch(
    ...
    session
        ...
        .expect("hello should apply");
    let playlist_dispatch = plan_local_runtime_dispatch(
        &session,
        0.0,
...
#[test]
fn plan_local_runtime_dispatch_passthroughs_simple_actions() {
