...
impl RuntimePlaybackCoordination {
    ...
    pub(crate) fn stage_local_pause_intent(&mut self, paused: bool, session: &ClientSession) {
        ...
        self.pending_local_pause_intent = Some(PendingLocalPauseIntent {
            ...
            preceding_local_transport: self
                .pending_local_transport_echo
                .as_ref()
                .filter(|predecessor| {
                    self.local_transport_scope_matches(predecessor, session)
                        && session.current_room_transport_revision()
                            == Some(predecessor.base_revision)
                })
                .cloned(),
            authorization,
...
pub(super) struct PendingLocalPauseIntent {
    ...
    pub(super) preceding_local_transport: Option<PendingLocalTransportEcho>,
