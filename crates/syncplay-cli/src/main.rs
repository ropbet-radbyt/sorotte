...
"#,
    ...
    #[tokio::test]
    async fn connected_client_session_non_space_delimited_known_tokens_do_not_emit_outbound_actions()
     {
        let listener = TcpListener::bind("127.0.0.1:0")
            .await
            .expect("listener should bind");
                ...
                if let ProtocolMessage::Set(ref payload) = message {
                    assert!(
                        payload.set.room.is_none()
                            ...
                            && payload.set.playlist_index.is_none()
                            && payload.set.controller_auth.is_none(),
                        "non-space-delimited known-token input should not emit local command set messages: {payload:?}"
                    );
                }
            }
    ...
    async fn connected_client_session_real_mpv_explicit_ipc_smoke_applies_inbound_server_playstate_fastforward_via_ping_forward_delay_to_real_player()
     {
        use std::process::{Child, Command, Stdio};
        use std::time::{Instant, SystemTime, UNIX_EPOCH};
        use syncplay_protocol::{PingPayload, PlaystatePayload, StatePayload};
        ...
        let unique = SystemTime::now()
            ...
            .as_millis();
        let pipe_path = format!(
            r"\\.\pipe\syncplay-rust-cli-state-fastforward-smoke-{}-{unique}",
            std::process::id()
        );
    ...
    async fn connected_client_session_real_mpv_explicit_ipc_smoke_borderline_fastforward_without_server_rtt_does_not_seek_real_player()
     {
        ...
        let unique = SystemTime::now()
            ...
            .as_millis();
        let pipe_path = format!(
            r"\\.\pipe\syncplay-rust-cli-state-fastforward-no-server-rtt-smoke-{}-{unique}",
            std::process::id()
        );
        ...
        let _mpv_child = MpvChildGuard(
            Command::new(&mpv_bin)
                ...
                .stderr(Stdio::null())
                .spawn()
                .expect("standalone mpv should start for explicit-IPC no-serverRtt smoke"),
        );
        let _warmup_attach = super::connect_mpv_adapter_with_retry(
            &pipe_path,
            Duration::from_secs(5),
        ...
        let final_position = runtime.player().position_seconds();
        assert!(
            final_position < target_position - 2.0,
            "expected borderline fastforward without serverRtt to avoid seeking real mpv near target; target={target_position}; position={final_position}; speed={}",
            runtime.player().playback_rate()
        );
    }
