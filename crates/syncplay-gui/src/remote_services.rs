...
pub(crate) fn fetch_public_servers(
    ...
    if let Some(body) = env_response_override(SYNCPLAY_PUBLIC_SERVER_LIST_RESPONSE_ENV) {
        return parse_public_server_response(&body);
    }
    ...
    fetch_public_servers_from_url(&url, language)
}
...
pub(crate) fn check_for_updates(
    ...
    if let Some(body) = env_response_override(SYNCPLAY_UPDATE_CHECK_RESPONSE_ENV) {
        return match parse_update_check_response(&body, user_initiated) {
            ...
            Err(error) => LegacyUpdateCheckResult {
                ...
                message: format!(
                    "{}\n-----\n{}",
                    error,
                    update_check_failed_notification_message()
                ),
...
fn fetch_update_check_result_from_url(
    ...
    parse_update_check_response(&body, user_initiated)
}
...
fn parse_update_check_response(
    ...
    let message = parsed
        ...
        .unwrap_or_else(|| default_update_check_message(&status));
    let mut url = parsed
        .get("version-url")
        .and_then(Value::as_str)
        .map(str::trim)
...
fn default_update_check_message(status: &LegacyUpdateCheckStatus) -> String {
    match status {
        LegacyUpdateCheckStatus::UpToDate => "Syncplay is up to date".to_owned(),
        LegacyUpdateCheckStatus::UpdateAvailable => {
...
fn update_check_failed_notification_message() -> String {
    format!(
...
mod tests {
    ...
    fn update_check_response_parser_falls_back_to_default_failure_message_for_unknown_status() {
        let parsed = parse_update_check_response(r#"{"version-status":"mystery"}"#, true)
        ...
        assert_eq!(
            parsed.message,
            default_update_check_message(&LegacyUpdateCheckStatus::Unknown("mystery".to_owned()))
        );
        assert_eq!(parsed.url.as_deref(), Some("https://syncplay.pl/download/"));
    }
