...
fn main() {
    ...
    match run_benchmark(&options) {
        ...
        Err(error) => {
            eprintln!("syncplay-gui-startup-bench failed: {error}");
            std::process::exit(1);
        }
    }
}
...
fn usage() -> &'static str {
    "usage: syncplay-gui-startup-bench --binary PATH [--samples N] [--warmup N] [--timeout-ms N] [--scenario NAME] [--keep-profile-copy] [--json] [--compare-to PATH] [--output-dir PATH]"
}
...
fn run_benchmark(options: &StartupBenchOptions) -> Result<StartupBenchmarkResults, String> {
    let binary_path = fs::canonicalize(&options.binary_path).map_err(|error| {
        format!(
            "failed to resolve syncplay-gui binary {}: {error}",
            options.binary_path.display()
        )
    })?;
    if !binary_path.is_file() {
        return Err(format!(
            "syncplay-gui binary is not a file: {}",
            binary_path.display()
        ));
    }
...
fn run_sample(request: StartupSampleRequest<'_>) -> Result<StartupRun, String> {
    ...
    let config_path = sample_root.join("syncplay-bench.ini");
    ...
    let mut child = match launch_syncplay_gui_with_retry(&driver, binary_path, launch, timeout) {
        Ok(child) => child,
        Err(error) => {
            if let Some(server) = mock_server {
                let _ = server.release();
...
fn copy_real_profile_for_sample(sample_root: &Path, config_path: &Path) -> Result<(), String> {
    let appdata = std::env::var_os("APPDATA")
        .map(PathBuf::from)
        .ok_or_else(|| {
            "profile-copy requires APPDATA to locate the real Syncplay profile".to_owned()
        })?;
    let source_config = appdata.join("syncplay.ini");
    if !source_config.is_file() {
        return Err(format!(
            "profile-copy could not find real Syncplay config {}",
            source_config.display()
        ));
    }
    copy_file_to_path(&source_config, config_path, "real Syncplay config")?;

    let source_main_window = appdata.join("Syncplay").join("MainWindow.ini");
    if source_main_window.is_file() {
        copy_file_to_path(
            &source_main_window,
            &sample_root.join("Syncplay").join("MainWindow.ini"),
            "real Syncplay main-window state",
        )?;
    }

    let source_stream_helper = appdata.join("Syncplay").join("tools").join("stream-helper");
...
fn cleanup_profile_copy_sample(sample_root: &Path, config_path: &Path) -> Result<(), String> {
    ...
    let syncplay_root = sample_root.join("Syncplay");
    match remove_dir_all_with_retries(&syncplay_root, 50, Duration::from_millis(100)) {
        Ok(()) => {}
        Err(error) if error.kind() == ErrorKind::NotFound => {}
        Err(error) => {
            return Err(format!(
                "failed to remove copied profile GUI state {}: {error}",
                syncplay_root.display()
            ));
        }
    }
    Ok(())
...
fn write_gui_active_view(root: &Path, active_view: &str) -> Result<(), String> {
    let path = root.join("Syncplay").join("MainWindow.ini");
...
fn launch_syncplay_gui(binary_path: &Path, launch: GuiLaunchConfig<'_>) -> Result<Child, String> {
    ...
    for name in [
        ...
        "SYNCPLAY_GUI_PUBLIC_SERVER_LIST_URL",
        "SYNCPLAY_GUI_PUBLIC_SERVER_LIST_RESPONSE",
        "SYNCPLAY_GUI_UPDATE_CHECK_URL",
        "SYNCPLAY_GUI_REFRESH_PUBLIC_SERVERS",
        "SYNCPLAY_GUI_REFRESH_PUBLIC_SERVERS_PATH",
    ] {
    ...
    if let Some(update_check_response) = launch.update_check_response {
        command.env("SYNCPLAY_GUI_UPDATE_CHECK_RESPONSE", update_check_response);
    }
    if let Some(public_servers_spec) = launch.public_servers_spec {
        command.env("SYNCPLAY_GUI_REFRESH_PUBLIC_SERVERS", public_servers_spec);
    }
    ...
    if let Some(tcp_session) = launch.tcp_session {
        ...
        command.env("SYNCPLAY_CLIENT_HOST", "127.0.0.1");
        command.env("SYNCPLAY_CLIENT_PORT", tcp_session.port.to_string());
        command.env("SYNCPLAY_CLIENT_USERNAME", BENCH_USERNAME);
        command.env("SYNCPLAY_CLIENT_ROOM", BENCH_ROOM);
    }
...
fn launch_syncplay_gui_with_retry<D: NativeGuiDriver>(
    ...
    for attempt in 1..=LAUNCH_ATTEMPTS {
        let mut child = launch_syncplay_gui(binary_path, GuiLaunchConfig { ..launch })?;
...
fn render_text_report(results: &StartupBenchmarkResults) -> String {
    ...
    out.push_str("Syncplay GUI startup benchmark\n");
...
mod tests {
    ...
    fn parse_options_default_scenarios_do_not_include_profile_copy() {
        let options = parse_options(&["--binary".to_owned(), "syncplay-gui.exe".to_owned()])
