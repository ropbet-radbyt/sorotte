...
const GUI_EXE: &str = "syncplay-gui.exe";
const UPDATER_EXE: &str = "syncplay-gui-updater.exe";
...
fn run_update(args: UpdaterArgs) -> Result<(), String> {
    ...
    append_log(&plan.log_path, "waiting for Syncplay GUI to exit")?;
    wait_for_process_exit(plan.pid)?;
    append_log(&plan.log_path, "applying staged update")?;
    apply_replacement_plan(&plan)?;
    if plan.restart {
        append_log(&plan.log_path, "restarting Syncplay GUI")?;
        Command::new(&plan.target_exe_path)
            .spawn()
            .map_err(|error| format!("failed to restart Syncplay GUI: {error}"))?;
    }
...
mod tests {
    ...
    fn relative_path_rejects_parent_components() {
        assert!(relative_path_is_safe(Path::new("syncplay-gui.exe")));
        assert!(!relative_path_is_safe(Path::new("../syncplay-gui.exe")));
    }
}
