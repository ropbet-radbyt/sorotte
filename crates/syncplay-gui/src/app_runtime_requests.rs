...
use super::super::shell_state::{
    ...
    MainWindowRuntimeSnapshot, SyncplayGuiShellAppState, browser_stream_target_kind,
};
    ...
    pub(super) fn handle_runtime_request(
        ...
        match request {
            ...
            GuiRuntimeRequest::RetryPlayerLaunch => {
                ...
                let stream_helper_snapshot =
                    self.recheck_stream_helper_runtime_snapshot(projected_state);
            ...
            GuiRuntimeRequest::InstallStreamHelper => {
                let Some(root) = self.legacy_gui_qsettings_root() else {
                    Self::push_runtime_error_notification(
                        handle,
                        projected_state,
                        "Stream helper installation requires a writable GUI config root."
                            .to_owned(),
                    );
                    return false;
                };
                self.report_stream_helper_remediation_progress(
                    ...
                    Some(
                        "Starting helper installation for extractor-backed media URLs.".to_owned(),
                    ),
                    0.02,
                );
                match install_or_update_managed_stream_helper_with_progress(&root, |progress| {
                    self.apply_stream_helper_remediation_progress(
                        handle,
                        projected_state,
                        progress,
                    );
                    ...
                    Ok(message) => {
                        self.finish_stream_helper_remediation_success(
                            handle,
                            projected_state,
                            message,
                        );
            ...
            GuiRuntimeRequest::IntegrateStreamHelperDownloader(source_path) => {
                ...
                self.report_stream_helper_remediation_progress(
                    ...
                    projected_state,
                    "Preparing stream helper remediation",
                    Some("Starting yt-dlp import into Syncplay's managed helper.".to_owned()),
                    0.02,
                );
            ...
            GuiRuntimeRequest::IntegrateStreamHelperJsRuntime(source_path) => {
                ...
                self.report_stream_helper_remediation_progress(
                    ...
                    projected_state,
                    "Preparing stream helper remediation",
                    Some("Starting Deno import into Syncplay's managed helper.".to_owned()),
                    0.02,
                );
            ...
            GuiRuntimeRequest::RecheckStreamHelper => {
                ...
                if snapshot.health == GuiStreamHelperHealth::Healthy {
                    let message =
                        "Stream helper support is ready for the current media URL.".to_owned();
                    actions.push(GuiShellAction::PushTransientNotification {
                        level: GuiTransientNotificationLevel::Success,
                        message: message.clone(),
                    });
