...
- The main parity blockers are now the remaining default desktop workflow gaps after connection exists: joining or switching rooms without lying to the user, getting files into a real shared playlist from normal desktop affordances, drag-and-drop ingest, and tightening affordances that still over-promise beyond the runtime-backed paths.

## Highest-Priority Remaining Work

### 1. Make Room Join/Leave Runtime-Authoritative Instead Of Optimistic Shell State
...
### 2. Replace Preview-Only Shared-Playlist File Opening With Real Runtime Dispatch
...
### 3. Implement Desktop Drag-And-Drop For Media And Playlist Ingest
...
### 4. Tighten Command Availability So The UI Stops Advertising Non-Working Paths
...
1. Make room join and leave reflect server-confirmed state rather than local optimism.
2. Turn shared-playlist file open and import into real runtime behavior.
3. Add drag-and-drop and cover it in native and semantic smoke tests.
4. Tighten command availability so non-working paths stop looking production-ready.
