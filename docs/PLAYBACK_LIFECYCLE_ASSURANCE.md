...
In lifecycle-capable sessions, a local playlist edit and its resulting index are sent as adjacent `playlistChange` and `playlistIndex` fields in one reliable `Set` frame. The server compares the selected media before and after the whole edit. Reordering or deleting another row preserves the current transport and preparation even when the active row moves; changing the selected media still resets it. Deleting the active row follows the compound edit with a separate explicit index request and records a local reset intent, including when the successor has the same label. Keeping the final index in the edit prevents a removed last row from temporarily retiring selection before the explicit reset. Both index echoes remain owned by the local operation. Same-label explicit selection also prepares a fresh V2 playback episode; predecessor physical evidence cannot satisfy its readiness. A standalone index request remains an explicit selection or same-row replay. Generic playlist replacement carries labels and an index, not physical row identity; callers selecting a different same-label source must express that selection explicitly rather than infer identity from duplicate counts. Connection-scoped playback `State` belongs to the receiver's current room; `setBy` is historical authorship and does not supply room scope.

Mixed rooms receive the same compatible playlist and transport fanout. An older client that sends the edit and index in separate frames cannot communicate that atomic intent: the server continues treating its standalone index as a selection, so such edits can still reset transport in a lifecycle-governed room. No time window suppresses a subsequent explicit replay. Rooms containing only Syncplay peers retain their independent playlist/transport behavior.
