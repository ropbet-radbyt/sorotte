...
In lifecycle-capable sessions, a local playlist edit and its resulting index are sent as adjacent `playlistChange` and `playlistIndex` fields in one reliable `Set` frame. The server compares the selected media before and after the whole edit. Reordering or deleting another row preserves the current transport and preparation even when the active row moves; changing the selected media still resets it. A standalone index request remains an explicit selection or same-row replay. Connection-scoped playback `State` belongs to the receiver's current room; `setBy` is historical authorship and does not supply room scope.
