...
Sorotte uses one source-independent playback coordinator for local files, Plex streams, direct network media, and extractor-backed URLs such as YouTube. The coordinator owns loading, cache stalls, command completion, bounded recovery, and start acknowledgements; GUI and CLI adapters only execute its decisions and forward observations.
...
`open_file`, pause, and seek commands use tracked IDs in production mpv paths. IPC acceptance means only that mpv accepted a command. Completion requires matching observations:

- pause: logical pause is observed;
- seek: seeking has ended and position is within tolerance;
- play: logical play, cache release, a newer playback restart, and forward position advancement are all observed;
...
A cache pause never changes the user's logical pause state. While loading, cache-paused, seeking, core-idle, or recovering, the coordinator blocks ordinary drift correction. This is the rule that prevents the old loop in which each new room timestamp caused another seek and discarded the buffer that had just filled.
...
- moderate lag uses gentle catch-up, capped by `streamingMaxCatchupRate`;
...
Explicit room pause, explicit room seek, and source replacement remain authoritative and supersede the old recovery target. Live/sliding seeks are clamped to the latest valid seekable range rather than sent beyond the window.
...
With a non-immediate policy, the initiating client starts `sorottePlaybackBarrierV1`. `continue` accepts the server's best-effort timeout commit. `remain-paused` immediately restores the room pause after a timed-out commit. `ask-controller` also keeps the room paused and presents a GUI warning so the controller can decide manually.
...
1. An authorized initiator sends `prepare` with an opaque logical ID, room generation, target, policy, quorum percentage when needed, and timeout.
2. The server pauses the canonical room, captures the currently capable cohort, excludes legacy peers, normalizes quorum and deadline, and broadcasts prepare/status.
3. A client reports `ready` only after the exact logical source is `ReadyPaused`, logically paused, not cache-paused/seeking/core-idle, within 0.5 seconds of the target, and the prepare-derived desired revision has been observed as applied.
4. When controller/all/quorum readiness is satisfied—or at the bounded deadline—the server creates one server-owned revision, commits, and sends the compatible room unpause.
5. A client reports `started` only after the committed revision has produced a playback restart and forward position advancement.
6. The server publishes `complete` or explicit participant degradation. Missing started acknowledgements time out after 10 seconds.

The server binds reports to authenticated connections; clients cannot name another participant. Stale generations/revisions, malformed values, unauthorized prepare/policy requests, duplicate peer prepares for an active logical source, and optimistic playstate bundled with an acknowledgement are ignored.
...
- protocol/client/server tests cover prepare/ready/commit/started, strict identity/revision validation, quorum rounding, mixed legacy rooms, authorization, disconnects, debounce/hysteresis, and fail-open timeouts;
...
- GUI semantic tests cover persistence and the configuration surface.
...
Nightly CI installs mpv and runs the ignored real-player fault harness:

```powershell
cargo test -p sorotte-sim --test mpv_rebuffer_harness -- --ignored --nocapture
```

The deterministic harness uses local HTTP media, not the public YouTube service or a live Plex server. External extractor availability, YouTube site changes, Plex transcoder behavior, and third-party network conditions remain suitable for opt-in smoke testing rather than required CI.
