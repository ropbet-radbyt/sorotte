...
| T08 | Versioned guest inputs, reviewed integrated Sandbox scripts, one-job runner controller, readiness before registration, watchdog and narrow teardown. | Actual controller-interruption and cancellation drills passed on `67679a7a`, with exact guest/job/runner ownership, retained diagnostics and completed watchdog receipts. That source's positive run exposed a missing-pip prerequisite before any GUI scenario. A shared offline Python probe now checks the copied interpreter, pinned dependency closure and imports before bundle publication or runner registration; full positive qualification of the repaired source and new inputs remains required. |
